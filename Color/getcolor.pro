...
;         if not keyword_set(decomposedState) then decomposedState=0 ;; Kludge for datareduc
      ...
      IF Keyword_Set(load) THEN BEGIN
         ...
         ;; Add in nice FSC+COLORS
         addn = ['red5', 'blu5', 'grn5']
         for ii=0L,n_elements(addn)-1 do begin
            returnValue = Create_Struct(returnValue, addn[ii], x_fsc_color(addn[ii]))
         endfor
         ;; Some fixes
         returnValue.mediumgray = x_fsc_color('SlateGray')
         RETURN, returnValue
      ENDIF
