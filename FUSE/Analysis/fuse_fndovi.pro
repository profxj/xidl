...
pro fuse_fndovi_icmmn, datfil, gzfil, STIS=stis

  common fuse_fndovi_cmm, $
    npix, $
    fx, $
    wv, $
    sig, $
  ...
  ;; Read FUSE data
  if not keyword_set( STIS ) then begin
      fx = x_readspec(datfil, inflg=3, sig=sig, wav=wv, npix=npix)
  endif else begin
      sp = strpos(datfil, 'f.fits')
      sigfil = strmid(datfil, 0, sp)+'e.fits'
      fx = x_readspec(datfil, sig=sig, wav=wv, npix=npix, fil_sig=sigfil)
  endelse
...
pro fuse_fndovi_event, ev
  ...
  case uval of
      'ZABS': begin
          ...
          state.zabs=tmp
          fuse_fndovi_update, state
      end
      'SAVE': fuse_fndovi_svgz, state
      'NEXT': begin
          fuse_fndovi_next, state
          fuse_fndovi_update, state
      end
      'PREV': begin
          fuse_fndovi_prev, state
          fuse_fndovi_update, state
      end
      'LDRAW_BASE' : begin
...
pro fuse_fndovi_spectra, state
  ...
  ;; H2
  oplot, (state.h2plotx/1031.9261)-1., state.h2ploty, color=clr.green, psym=10, $
    linestyle=1
  ...
  ;; H2
  oplot, (state.h2plotx/1037.6167)-1., state.h2ploty, color=clr.green, psym=10, $
    linestyle=1
...
;;;;;;
; UPDATE LYA+METALS
pro fuse_fndovi_update, state
;  fuse_fndovi_updfit, state
  widget_control, state.zabs_id, set_value=state.zabs
  fuse_fndovi_setxym, state
  fuse_fndovi_spectra, state
;  fuse_fndovi_Metals, state
  return
end

;;;;;;;;;;;;;;;
;;; SET XY LIM
pro fuse_fndovi_setxym, state
...
pro fuse_fndovi, datfil, STIS=stis, $
                 XSIZE=xsize, YSIZE=ysize, OUTFIL=outfil, $
                 GZFIL=gzfil
...
;
  if  N_params() LT 1  then begin 
    print,'Syntax - ' + $
      'fuse_fndovi, datfil, /STIS, OUTFIL='
    print, '        XSIZE=, YSIZE=, GZFIL=, [v1.1]'
    return
  endif 
...
;  if not keyword_set( YSIZE ) then ysize = 800
  ...
  if not keyword_set( OUTFIL ) then begin
      if keyword_set( GZFIL ) then outfil= gzfil else $
        outfil = 'find_ovi.fits'
  endif

; Initialize the common blcok
  fuse_fndovi_icmmn, datfil, gzfil, STIS=stis

  if not keyword_set( STRTZ) then strtz = (wv[0]+1.)/1031.9261 - 1.
...
; STATE
  ...
  state = {             $
            ...
            zabs: strtz, $
            dz: deltaz, $
            z1arr: (wv/1031.9261)-1., $
            z2arr: (wv/1037.6167)-1., $
            ngz: ngz, $
            outfil: outfil, $
            patt: bytarr(10,10), $
            h2lines: h2lines, $
...
;    WIDGET
  base = WIDGET_BASE( title = 'fuse_fndovi: Find OVI', /row, $
...
;      BUTTONS
  ...
  state.zabs_id = cw_field(butbase, title='zabs: ', value=0., $
                           xsize=5, /return_events, UVALUE='ZABS')
