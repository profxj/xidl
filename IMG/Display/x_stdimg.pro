...
;
; CALLING SEQUENCE:
;       x_stdimg [,array_name OR fits_file] [,min = min_value] [,max=max_value] 
;           [,/linear] [,/log] [,/histeq] [,/block]
;           [,/align] [,/stretch] [,header = header]
;
; REQUIRED INPUTS:
;       None.  If x_stdimg is run with no inputs, the window widgets
;       are realized and images can subsequently be passed to x_stdimg
...
; 
; COMMON BLOCKS:
;       x_stdimg_state:  contains variables describing the display state
;       x_stdimg_images: contains the internal copies of the display image
;       x_stdimg_color:  contains colormap vectors
;       x_stdimg_pdata:  contains plot and text annotation information
;
; RESTRICTIONS:
...
;
; EXAMPLE:
;       To start x_stdimg running, just enter the command 'x_stdimg' at the idl
;       prompt, either with or without an array name or fits file name 
;       as an input.  Only one x_stdimg window will be created at a time,
;       so if one already exists and another image is passed to x_stdimg
;       from the idl command line, the new image will be displayed in 
;       the pre-existing x_stdimg window.
;
...
;              http://cfa-www.harvard.edu/~abarth/x_stdimg/x_stdimg.html
;       Revised by JXP Aug-2001
;
;-
;----------------------------------------------------------------------
;        x_stdimg startup, initialization, and shutdown routines
;----------------------------------------------------------------------

pro x_stdimg_initcommon

; Routine to initialize the x_stdimg common blocks.  Use common blocks so
; that other IDL programs can access the x_stdimg internal data easily.

common x_stdimg_state, state, flg_img
common x_stdimg_color, r_vector, g_vector, b_vector
common x_stdimg_pdata, nplot, maxplot, plot_ptr
common x_stdimg_images, $
  main_image, $
  display_image, $
  scaled_image,  $
  wave_image, $
...
state = {                   $
          ...
          active_window_id: 0L, $        ; user's active window outside x_stdimg
...
pro x_stdimg_startup

; This routine initializes the x_stdimg internal variables, and creates and
; realizes the window widgets.  It is only called by the x_stdimg main
; program level, when there is no previously existing x_stdimg window.

common x_stdimg_state
common x_stdimg_color
...
base = widget_base(title = 'x_stdimg', $
                   /column, /base_align_right, $
                   app_mbar = top_menu, $
                   uvalue = 'x_stdimg_base', $
                   /tlb_size_events)
state.base_id = base
...
widget_control, top_menu, event_pro = 'x_stdimg_topmenu_event'
widget_control, state.draw_widget_id, event_pro = 'x_stdimg_draw_color_event'
widget_control, state.draw_base_id, event_pro = 'x_stdimg_draw_base_event'
widget_control, state.keyboard_text_id, event_pro = 'x_stdimg_keyboard_event'
widget_control, state.pan_widget_id, event_pro = 'x_stdimg_pan_event'
...
; See the routine x_stdimg_stretchct to see why we do it this way.
...
pro x_stdimg_colorbar

; Routine to tv the colorbar at the bottom of the x_stdimg window

common x_stdimg_state

x_stdimg_setwindow, state.colorbar_window_id
...
pro x_stdimg_shutdown, windowid

; routine to kill the x_stdimg window(s) and clear variables to conserve
; memory when quitting x_stdimg.  The windowid parameter is used when
; x_stdimg_shutdown is called automatically by the xmanager, if x_stdimg is
; killed by the window manager.

common x_stdimg_images
common x_stdimg_state
common x_stdimg_color
common x_stdimg_pdata

; Kill top-level base if it still exists
if (xregistered ('x_stdimg')) then widget_control, state.base_id, /destroy

; Destroy all pointers to plots and their heap variables
if (nplot GT 0) then begin
    x_stdimgerase, /norefresh
endif
...
;--------------------------------------------------------------------
;                  main x_stdimg event loops
;--------------------------------------------------------------------

pro x_stdimg_topmenu_event, event
...
; of x_stdimg window without entering the main base
x_stdimg_getwindow
...
; File menu options:
    'ReadFits': begin
        x_stdimg_readfits, newimage=newimage
        if (newimage EQ 1) then begin
            x_stdimg_getstats
            x_stdimg_settitle
            state.zoom_level =  0
            state.zoom_factor = 1.0
            if (state.default_autoscale EQ 1) then x_stdimg_autoscale
            x_stdimg_set_minmax
            x_stdimg_displayall
        endif
    end
    'WritePS' : x_stdimg_writeps
    'WriteTiff': x_stdimg_writetiff
    'Quit':     x_stdimg_shutdown
; ColorMap menu options:            
    'Grayscale': x_stdimg_getct, 0
    'Blue-White': x_stdimg_getct, 1
    'Red-Orange': x_stdimg_getct, 3
    'BGRY': x_stdimg_getct, 4
    'Rainbow': x_stdimg_getct, 13
    'Stern Special': x_stdimg_getct, 15
    'Green-White': x_stdimg_getct, 8
    'ATV Special': x_stdimg_makect, event_name
; Scaling options:
    'Linear': begin
        state.scaling = 0
        x_stdimg_displayall
...
; Label options:
    'TextLabel': x_stdimg_textlabel
    'Contour': x_stdimg_oplotcontour
    'Compass': x_stdimg_setcompass
    'ScaleBar': x_stdimg_setscalebar
    'EraseLast': x_stdimgerase, 1
    'EraseAll': x_stdimgerase
; Blink options:
    'SetBlink1': begin   
        x_stdimg_setwindow, state.draw_window_id
...
; Info options:
    'Photometry': x_stdimg_apphot
    'ImageHeader': x_stdimg_headinfo
    'Statistics': x_stdimg_showstats

; Help options:            
    'ATV Help': x_stdimg_help
    
    else: print, 'Unknown event in file menu!'
endcase

; Need to test whether x_stdimg is still alive, since the quit option
; might have been selected.        
if (xregistered('x_stdimg', /noshow)) then x_stdimg_resetwindow

end

;--------------------------------------------------------------------

pro x_stdimg_draw_color_event, event
...
case event.type of
    0: begin           ; button press
        if (event.press EQ 1) then begin
            state.cstretch = 1
            x_stdimg_stretchct, event.x, event.y, /getmouse
            x_stdimg_colorbar
        endif else begin
            x_stdimg_zoom, 'none', /recenter
        endelse
    end
    1: begin
        state.cstretch = 0  ; button release
        if (state.bitdepth EQ 24) then x_stdimg_refresh
        x_stdimg_draw_motion_event, event
    end
    2: begin                ; motion event
        if (state.cstretch EQ 1) then begin
            x_stdimg_stretchct, event.x, event.y, /getmouse 
            if (state.bitdepth EQ 24) then x_stdimg_refresh, /fast
        endif else begin 
            x_stdimg_draw_motion_event, event
        endelse
    end 
...
pro x_stdimg_draw_zoom_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: x_stdimg_zoom, 'in', /recenter
        2: x_stdimg_zoom, 'none', /recenter
        4: x_stdimg_zoom, 'out', /recenter
    endcase
endif

if (event.type EQ 2) then x_stdimg_draw_motion_event, event
...
pro x_stdimg_draw_blink_event, event
...
case event.type of
    ...
    2: x_stdimg_draw_motion_event, event ; motion event
endcase
...
pro x_stdimg_draw_motion_event, event
...
pro x_stdimg_draw_base_event, event
...
pro x_stdimg_keyboard_event, event
...
case eventchar of
    '1': x_stdimg_move_cursor, eventchar
    '2': x_stdimg_move_cursor, eventchar
    '3': x_stdimg_move_cursor, eventchar
    '4': x_stdimg_move_cursor, eventchar
    '6': x_stdimg_move_cursor, eventchar
    '7': x_stdimg_move_cursor, eventchar
    '8': x_stdimg_move_cursor, eventchar
    '9': x_stdimg_move_cursor, eventchar
    'r': x_stdimg_rowplot
    'c': x_stdimg_colplot
    's': x_stdimg_surfplot
    't': x_stdimg_contourplot
    'p': x_stdimg_apphot
    'i': x_stdimg_showstats
    'q': x_stdimg_shutdown
;  Modified 6-23-01 JXP
    'z': x_stdimg_zoom, 'in', /recenter
    'Z': x_stdimg_zoom, 'out', /recenter
;  Wavelength options
    'S': x_stdimg_specplot
;  Big pan
    '{': x_stdimg_bigpan, 'up'
    '}': x_stdimg_bigpan, 'down'
    '[': x_stdimg_bigpan, 'left'
    ']': x_stdimg_bigpan, 'right'
    else:  ;any other key press does nothing
endcase

if (xregistered('x_stdimg', /noshow)) then $
...
case event.type of
    ...
    2: begin
        x_stdimg_pantrack, event     ; motion event
        widget_control, state.pan_widget_id, /clear_events
    end
    else:
endcase
...
pro x_stdimg_event, event

; Main event loop for x_stdimg top-level base, and for all the buttons.
...
case uvalue of
    ...
    'mode': case event.index of
        0: widget_control, state.draw_widget_id, $
          event_pro = 'x_stdimg_draw_color_event'
        1: widget_control, state.draw_widget_id, $
          event_pro = 'x_stdimg_draw_zoom_event'
        2: widget_control, state.draw_widget_id, $
          event_pro = 'x_stdimg_draw_blink_event'
        3: widget_control, state.draw_widget_id, $
          event_pro = 'x_stdimg_draw_phot_event'
    ...
    'invert': begin                  ; invert the color table
        ...
        x_stdimg_stretchct, state.brightness, state.contrast
        if (state.bitdepth EQ 24) then x_stdimg_refresh
    end
    
    'restretch_button': x_stdimg_restretch

    'min_text': begin     ; text entry in 'min = ' box
        x_stdimg_get_minmax, uvalue, event.value
        x_stdimg_displayall
    end
    ...
    'zoom_in':  x_stdimg_zoom, 'in'         ; zoom buttons
    'zoom_out': x_stdimg_zoom, 'out'
    'zoom_one': x_stdimg_zoom, 'one'
...
;   Frame Number
    ...
    'frame_bg' : x_stdimg_setframe, event.value

    'done':  x_stdimg_shutdown
...
pro x_stdimg_message, msg_txt, msgtype=msgtype, window=window
...
;-----------------------------------------------------------------------
;      main x_stdimg routines for scaling, displaying, cursor tracking...
;-----------------------------------------------------------------------

pro x_stdimg_displayall
...
; passed to x_stdimg.  If the display image has just been moved around or
; zoomed without a change in scaling, then just call x_stdimg_refresh
...
pro x_stdimg_refresh, fast = fast
...
x_stdimg_setwindow, state.pan_pixmap
erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
x_stdimg_resetwindow

x_stdimg_setwindow, state.pan_window_id
if (not(keyword_set(fast))) then erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
x_stdimg_resetwindow
x_stdimg_drawbox, /norefresh

if (state.bitdepth EQ 24) then x_stdimg_colorbar

; redisplay the tracking image
if (not(keyword_set(fast))) then x_stdimg_gettrack
...
pro x_stdimg_move_cursor, direction
...
pro x_stdimg_get_minmax, uvalue, newvalue
...
pro x_stdimg_zoom, zchange, recenter = recenter
common x_stdimg_state
...
case zchange of
    ...
    else:  print,  'problem in x_stdimg_zoom!'
endcase
...
; Routine to pan in a large way on the image
  ...
  case direct of
      ...
      else:  print,  'problem in x_stdimg_bigpan!'
  endcase
...
x_stdimg_setwindow, state.track_window_id
tv, track_image
...
if (state.wcstype EQ 'angle') then begin
    ...
    wcsstring = x_stdimg_wcsstring(lon, lat, (*state.astr_ptr).ctype,  $
...
if (state.wcstype EQ 'lambda') then begin
    wavestring = x_stdimg_wavestring()
...
pro x_stdimg_drawbox, norefresh=norefresh
...
pro x_stdimg_getstats, align=align
...
pro x_stdimg_setwindow, windowid
...
; (i.e. non-x_stdimg) idl window.  Use x_stdimg_setwindow to set the window to
; one of the x_stdimg window, then display something to that window, then
; use x_stdimg_resetwindow to set the current window back to the currently
...
; graphics to the previous x_stdimg window actually get displayed to screen
...
pro x_stdimg_readfits, fitsfilename=fitsfilename, newimage=newimage
...
if (n_elements(strcompress(head, /remove_all)) LT 2) then begin
    x_stdimg_message, 'File does not appear to be a valid fits image!', $
...
if (numext EQ 0 AND naxis LT 2) then begin
    x_stdimg_message, 'Selected file is not a 2-d FITS image!', $
      window = window, msgtype = 'error'
    return
endif
...
if ((numext GT 0) AND (instrume NE 'WFPC2')) then begin
    x_stdimg_fitsext_read, fitsfile, numext, head, cancelled
endif else if ((instrume EQ 'WFPC2') AND (naxis EQ 3)) then begin
    x_stdimg_wfpc2_read, fitsfile, head, cancelled
endif else if ((naxis EQ 3) AND (origin EQ '2MASS')) then begin
    x_stdimg_2mass_read, fitsfile, head, cancelled
endif else begin
    x_stdimg_plainfits_read, fitsfile, head, cancelled
endelse
...
pro x_stdimg_fitsext_read, fitsfile, numext, head, cancelled
...
if (xten EQ 'BINTABLE') then begin
    x_stdimg_message, 'File appears to be a FITS table, not an image.', $
...
pro x_stdimg_plainfits_read, fitsfile, head, cancelled
...
fname = strcompress(state.current_dir + 'x_stdimg.ps', /remove_all)
...
pro x_stdimg_stretchct, brightness, contrast,  getmouse = getmouse
...
function x_stdimg_polycolor, p

; Routine to return an vector of length !d.table_size-8,
; defined by a 5th order polynomial.   Called by x_stdimg_makect
...
function x_stdimg_icolor, color
...
if (state.imagename EQ '') then begin
    widget_control, state.base_id, tlb_set_title = 'x_stdimg'
endif else begin
    ...
    title = strcompress('x_stdimg:  '+ name + '  ' + state.title_extras)
...
pro x_stdimg_setheader, head

; Routine to keep the image header using a pointer to a 
; heap variable.  If there is no header (i.e. if x_stdimg has just been
...
if (xregistered('x_stdimg_headinfo')) then begin
    widget_control, state.headinfo_base_id, /destroy
endif

if (xregistered('x_stdimg_stats')) then begin
...
if (not(ptr_valid(state.head_ptr))) then begin
    ...
    x_stdimg_message, 'No header information available for this image!', $
      msgtype = 'error', /window
    return
endif
...
if (not(xregistered('x_stdimg_headinfo', /noshow))) then begin
    ...
    headinfo_base = $
      widget_base(/floating, $
                  ...
                  /column, $
                  title = 'x_stdimg image header information', $
    ...
    xmanager, 'x_stdimg_headinfo', headinfo_base, /no_block
...
pro x_stdimg_headinfo_event, event
...
pro x_stdimg_plot1contour, iplot
common x_stdimg_pdata
common x_stdimg_state
...
pro x_stdimg_plot1compass, iplot
...
pro x_stdimg_plot1scalebar, iplot
...
x_stdimg_arcbar, *(state.head_ptr), $
...
for iplot = 1, nplot do begin
    case (*(plot_ptr[iplot])).type of
        'points'  : x_stdimg_plot1plot, iplot
        'text'    : x_stdimg_plot1text, iplot
        'contour' : x_stdimg_plot1contour, iplot
        'compass' : x_stdimg_plot1compass, iplot
        'scalebar': x_stdimg_plot1scalebar, iplot
        else      : print, 'Problem in x_stdimg_plotall!'   
    endcase
endfor
...
pro x_stdimgplot, x, y, _extra = options
common x_stdimg_pdata
common x_stdimg_state
...
if (not(xregistered('x_stdimg', /noshow))) then begin
...
;  convert color names to index numbers, and set default=red
   ...
   options.color = x_stdimg_icolor(options.color)
...
pro x_stdimgxyouts, x, y, text, _extra = options
common x_stdimg_pdata
common x_stdimg_state
...
pro x_stdimgcontour, z, x, y, _extra = options
common x_stdimg_pdata
common x_stdimg_state
...
; need not be the same dataset displayed in the x_stdimg window, but it
...
;  convert color names to index numbers, and set default=red
   ...
   options.c_color = x_stdimg_icolor(options.c_color)
...
pro x_stdimgerase, nerase, norefresh = norefresh
common x_stdimg_pdata
...
if (NOT keyword_set(norefresh)) then x_stdimg_refresh

end

;----------------------------------------------------------------------

pro x_stdimg_textlabel

; widget front end for x_stdimgxyouts
...
; switch red and black indices
    ...
    x_stdimgxyouts, textform.tag1, textform.tag2, textform.tag0, $
...
pro x_stdimg_oplotcontour

; widget front end for x_stdimgcontour
...
; switch red and black indices
    ...
    x_stdimgcontour, main_image, c_color = labelcolor, $
...
if (nplot GE maxplot) then begin
    x_stdimg_message, 'Total allowed number of overplots exceeded.', $
...
if (state.wcstype NE 'angle') then begin 
    x_stdimg_message, 'Cannot get coordinate info for this image!', $
      mesgtype = 'error', /window
    return
endif
...
cform = cw_form(formdesc, /column, $
                   title = 'x_stdimg scalebar properties')
...
xmanager, 'x_stdimg_lineplot', state.lineplot_base_id, /no_block
...
pro x_stdimg_rowplot

common x_stdimg_state
common x_stdimg_images

;if (not (xregistered('x_stdimg_lineplot', /noshow))) then begin
;    x_stdimg_lineplot_init
;endif

;x_stdimg_setwindow, state.lineplot_window_id
;erase
...
pro x_stdimg_lineplot_event, event
...
i = i + 1
h[i] = '    q: quits x_stdimg'
i = i + 2
h[i] = 'IDL COMMAND LINE HELP:'
i = i + 1
h[i] =  'To pass an array to x_stdimg:'
i = i + 1
h[i] =  '   x_stdimg, array_name [, options]'
i = i + 1
h[i] = 'To pass a fits filename to x_stdimg:'
i = i + 1
h[i] = '    x_stdimg, fitsfile_name [, options] (enclose filename in single quotes) '
i = i + 1
...
i = i + 1
h[i] = '    x_stdimgcontour, array_name [, options...]'
i = i + 1
h[i] = 'To overplot text on the draw window: '
i = i + 1
h[i] = '    x_stdimgxyouts, x, y, text_string [, options]  (enclose string in single quotes)'
i = i + 1
h[i] = 'To overplot points or lines on the current plot:'
i = i + 1
h[i] = '    x_stdimgplot, xvector, yvector [, options]'
i = i + 2
h[i] = 'The options for x_stdimgcontour, x_stdimgxyouts, and x_stdimgplot are essentially'
i = i + 1
...
h[i] = 'Other commands:'
i = i + 1
h[i] = 'x_stdimgerase [, N]:       erases all (or last N) plots and text'
i = i + 1
h[i] = 'x_stdimg_shutdown:   quits x_stdimg'
i = i + 1
h[i] = 'NOTE: If x_stdimg should crash, type x_stdimg_shutdown at the idl prompt.'
i = i + 5
h[i] = strcompress('ATV.PRO version '+state.version)
i = i + 1
h[i] = 'For full instructions, or to download the most recent version, go to:'
i = i + 1
h[i] = 'http://cfa-www.harvard.edu/~abarth/x_stdimg/x_stdimg.html'


if (not (xregistered('x_stdimg_help', /noshow))) then begin

helptitle = strcompress('x_stdimg v' + state.version + ' help')
    ...
    xmanager, 'x_stdimg_help', help_base, /no_block
...
pro x_stdimg_stats_event, event
...
if (not (xregistered('x_stdimg_stats', /noshow))) then begin
    ...
    stats_base = $
      widget_base(/floating, $
                  ...
                  /column, $
                  /base_align_center, $
                  title = 'x_stdimg image statistics', $
                  uvalue = 'stats_base')
    ...
    xmanager, 'x_stdimg_stats', stats_base, /no_block
...
x_stdimg_setwindow, state.statzoom_window_id
erase
...
;--------------------------------------------------------------------
;    x_stdimg main program.  needs to be last in order to compile.
;---------------------------------------------------------------------

; Main program routine for ATV.  If there is no current ATV session,
; then run x_stdimg_startup to create the widgets.  If ATV already exists,
...
if ( (n_params() EQ 0) AND (xregistered('x_stdimg', /noshow))) then begin
    print, 'USAGE: x_stdimg, array_name OR fitsfile, '
...
if not(xregistered('x_stdimg',/noshow)) then x_setobjgui_shutdown, windowid
...
; Before starting up x_stdimg, get the user's external window id.  We can't
; use the x_stdimg_getwindow routine yet because we haven't run x_stdimg
; startup.  A subtle issue: x_stdimg_resetwindow won't work the first time
...
if ( (n_params() NE 0) AND (size(image, /tname) EQ 'STRING')) then begin
    ...
    if (count EQ 0) then begin
        ...
        x_stdimg_readfits, fitsfilename=image, newimage=newimage
    endelse
endif
...
; If user has passed x_stdimg a data array, read it into main_image.
if ( (n_params() NE 0) AND (size(image, /tname) NE 'STRING') AND $
...
if (not(xregistered('x_stdimg', /noshow))) then begin
    nb = abs(block - 1)
    xmanager, 'x_stdimg', state.base_id, no_block = nb, cleanup = 'x_stdimg_shutdown'
    wset, userwindow
    ; if blocking mode is set, then when the procedure reaches this
    ; line x_stdimg has already been terminated.  If non-blocking, then
