...
;
; PURPOSE:
;    Calculate kinematic characteristics for civ-ion transitions
...
pro ism_civkin, dla, nn, KBIN=kbin, PER=per, CHK=chk, SPEC_ROOT=spec_root 

  if (N_params() LT 2) then begin 
    print,'Syntax - ' + $
             'ism_civkin, dla, nn  [v1.0]'
    return
 endif 
  ...
  if dla[nn].flgciv LE 0 then return
  c = x_constants()

  ;; Read in data
  if dla[nn].flgciv GT 1 then flg = dla[nn].flgciv else flg = 0
  dat = x_readspec(SPEC_ROOT+dla[nn].civfil, /struct, /autofsig, INFLG=flg)

  
  ;; Create the velo array
  x_pixminmax, dat.wv, dla[nn].civwav, dla[nn].zabs, dla[nn].civvmn,  $
               dla[nn].civvmx,  PIXMIN=pmn, PIXMAX=pmx, VELO=velo
  ...
  if max(badzero)-min(badzero) ge 5 then begin
     print, 'ism_civkin: too many or too large sections of bad data'
     stop
  endif
...
;  nbin = 7L
  ...
  if ngd EQ 0 then begin
      print, 'ism_civkin: Profile too saturated'
      stop
  endif
  ...
  ;; Evaluate and save
  dla[nn].civfvel = float(round(abs(velo[pix[rgt]]-velo[pix[lft]])))
  if keyword_set(CHK) then $
    print, 'ism_civkin: Velocity interval = ', dla[nn].civfvel

  ;;;;;
  ;; Mean/Median
  vcen = (velo[pix[rgt]]+velo[pix[lft]])/2.
  mean = dla[nn].civfvel/2.
  mn = min(abs(cumtau-0.5), imn)

  dla[nn].civfmm = abs( (velo[pix[imn]]-vcen)/mean )
  if keyword_set(CHK) then print, 'ism_civkin: Mean median = ', dla[nn].civfmm
  
  ;; Peak
  mx = max(ftau,imx)
;  mean = (velo[pix[rgt]]+velo[pix[lft]])/2.
  dla[nn].civfedg = abs( (velo[pix[imx]]-vcen) / mean )
  if keyword_set(CHK) then print, 'ism_civkin: Edge test = ', dla[nn].civfedg
  ...
  if keyword_set(CHK) then begin
      ...
      xyouts, velo[pix[lft]]+dv, 0.05, $
              'Dv = '+string(dla[nn].civfvel,format='(f5.1)')+' km/s',$
