...
; CALLING SEQUENCE:
;   
;  hires_nrmflat, hires, setup, [side], /CHK, /CLOBBER, /DEBUG, SCATT=, XSEP=
;
; INPUTS:
...
; OPTIONAL KEYWORDS:
;  XSEP=       --  [Default: -1]
;  /NOSCATT -- Do not subtract scattered light image
;
; OPTIONAL OUTPUTS:
...
pro hires_nrmflat, hires, setup, chip, CHK=chk, CLOBBER=clobber, DEBUG=debug, $
                  SCATT=scatt_img, XSEP=xsep, NOSCATT=noscatt

;
  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'hires_nrmflat, hires, setup, [chip], /CLOBBER,'
...
; Loop on chip
  ...
  for ii=0L,n_elements(chip)-1 do begin
      qq = chip[ii]
      idx = where(hires.setup EQ setup and hires.flg_anly NE 0 and $
                  hires.chip EQ qq and hires.type EQ 'TFLT',nidx)
      if nidx EQ 0 then stop
      rbin = hires[idx[0]].rowbin
      ;; Vignetting due to CCD cover
      upedg = round(3990./rbin)   ;; won't do anything for Single chip
      ;; Chip
      case qq of 
          -1: begin
              print, 'hires_nrmflat: Normalizing Single Chip'
              nxbkpt=2L 
              nybkpt=2L
          end
          1: begin
              print, 'hires_nrmflat: Normalizing BLUE Flat' 
              nxbkpt=5L 
              nybkpt=5L
          end
          2: begin
              print, 'hires_nrmflat: Normalizing GREEN Flat' 
              nxbkpt=5L 
              nybkpt=5L
          end
          3: begin
              print, 'hires_nrmflat: Normalizing RED Flat' 
              nxbkpt=5L 
              nybkpt=5L
          end
          else: stop
      ...
      ;; Check output file
      out_fil = hires_getfil('nqtz_fil', setup, CHIP=qq, /name, CHKFIL=chkf)
      if CHKF NE 0 AND not keyword_set( CLOBBER ) then begin
          print, 'hires_nrmflat: Normalized flat exists, moving on..'
          continue
      endif

      ;; Read in flat 
      flat_fil = hires_getfil('qtz_fil', setup, CHIP=qq,/name)
...
;          stop
      ...
      ;; Write 
      print, 'hires_nrmflat: Writing ', out_fil
