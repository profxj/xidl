...
pro deimos_qckrdx, img, indx, spec, sig, CLINE=cline, APER=aper, FLAT=flat, $
                   ...
                   FULL=full, SAPER=saper, NOPROC=noproc, nsky=nsky


;
  if  N_params() LT 3  then begin 
    print,'Syntax - ' + $
      'lris_qckredux, img, indx, spec, sig, CLINE=, APER=, ' + $
      'SKYFUNC=, SKYNORD=, '
    print, '        SKYREG=, /DISPLAY, /NOREJ, FLAT=, TRACE=, /NOSKY, STRCT=,'
...
;  Optional Keywords
  ...
  if not keyword_set(RN) then rn = 4.
  if not keyword_set(NSKY) then nsky = [30., 30.]
  if not keyword_set(FULL) then icut = 200L ELSE icut = 275L

; Process
  print, 'deimos_qckrdx: Processing the image'
  if not keyword_set(NOPROC) then $
    deimos_longproc, img, indx, finimg, FLAT=flat $
  else finimg = x_readimg(img)
...
; OBJLIN
  ...
  if keyword_set( strct ) then begin
      ...
      ; CLINE
      if not keyword_set( CLINE ) then  cline = 2048L
  endelse
