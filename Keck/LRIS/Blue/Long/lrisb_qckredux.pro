...
pro lrisb_qckredux, img, flat, spec, sig, CLINE=cline, APER=aper, $
                    ...
                    FULL=full, SAPER=saper, NOFLAT=noflat, NOPROC=noproc, $
                    FIMG=fimg
...
; Process
  ...
  if not keyword_set(NOPROC) then begin
      lrisb_proc, img, flat, finimg, /LONG, FULL=full, NOFLAT=noflat
  endif else begin
      finimg= xmrdfits(img, /sile)
  endelse
  sz = size(finimg, /dimensions)

; OBJLIN
  if not keyword_set(objlin) then begin
      if keyword_set( FULL ) then objlin = 150. else objlin = 104.
  endif
