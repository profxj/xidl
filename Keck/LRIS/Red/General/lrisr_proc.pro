...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro lrisr_proc, img, flat, finimg, LONG=long, FITS=fits, FULL=full, $
                NOFLAT=noflat


  if  N_params() LT 3  then begin 
      print,'Syntax - ' + $
        'lrisr_proc, img, flat, finimg, /LONG, FITS=, /FULL [v1.1]'
      return
  endif 
  
;  Optional Keywords
  if not keyword_set( GAIN ) then gain = 1.9
...
; Bias subtract
  ...
  lrisr_subbias, raw, ovimg, FULL=full

; Add in the gain
  ovimg = ovimg * gain

  ;; Transpose  (have the spectra go vertically)


; Flatten
  print, 'lrisr_proc: Flattening'
...
; Output
  if keyword_set( FITS ) then begin
      print, 'Writing ', FITS
      mwrfits, finimg, fits, /create
  endif

  print, 'lrisr_proc: All done!'
