...
;-
;------------------------------------------------------------------------------
pro sdss_goz, qalfil, outfil, XZFIL=xzfil, $
 ...
 ;;read in spectra
  ...
  for i=istrt,ntrials-1 do begin
      ...
      msk2 = msk
      vel = (wave/(1030.*(1+x[i].z_qso)) - 1.)*2.997924d5


      pixlyb = where(abs(vel) LT 6000., npix)
...
;      if x[i].plate EQ 286 and x[i].fiberid EQ 183 then stop
  ...
  ;;plot
  if keyword_set(PLOT) then x_splot,f,g,/block
  maxg=max(g)
;stop
  old_device=!d.name
  set_plot,'ps'
