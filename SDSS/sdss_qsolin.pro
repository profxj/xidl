...
pro sdss_qsolin, sdss_list, LSNR=lsnr, CCHK=cchk, PSFIL=psfil, $
  ...
  if  N_params() LT 1  then begin 
    print,'Syntax - ' + $
             'parse_sdssqso, sdss_list, PSFIL=, OUTDIR=, [v1.0]'
    return
  endif 
...
; LOOP
  ...
  for qq=istrt,nfil-1 do begin
      ...
      ;; REDO?
      if x_chkfil(outfil+'*', /silent) NE 0 AND not keyword_set( REDO ) then $
        continue

      print, 'sdss_qsolin: qq ', qq
      ;; rlam
      ...
      if ipend GT 100 then begin
          ...
          if keyword_set( CCHK) then x_splot, $
            flux[ipstrt:ipend], ytwo=conti, /block
