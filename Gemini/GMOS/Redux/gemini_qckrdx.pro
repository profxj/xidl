...
pro gemini_qckrdx, img, flat, spec, sig, CLINE=cline, APER=aper, $
                  ...
                  NSKY=nsky, FULL=full, SAPER=saper, _EXTRA=extra
...
;  Optional Keywords
  ...
  if not keyword_set(NSKY) then nsky = [70., 70.]
  if not keyword_set(FULL) then icut = 200L ELSE icut = 275L

; Process
  print, 'gemini_qckredux: Processing the image'
;  lrisb_proc, img, flat, finimg, /LONG, FULL=full
  finimg = xmrdfits(img, /silent)
  finimg = finimg*gain

  if keyword_set(FLAT) then finimg = finimg/flat
...
; OBJLIN
  if not keyword_set(objlin) then begin
      if keyword_set( FULL ) then objlin = 150. else objlin = 1159.
  endif
  ...
  if keyword_set( strct ) then begin
      ...
      ; CLINE
      if not keyword_set( CLINE ) then  cline = 375L
  endelse
