...
; CALLING SEQUENCE:
;   
;   uves_specplt, ydat, [head], XSIZE=, YSIZE=, TITLE=, WAVE=, LLIST=,
;           QAL=, ERR=, /GAL
;
; INPUTS:
;   ydat       - Values 
...
common uves_specplt_cmmn, uvesobj
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : uves_specplt_SetLine, state   ; Set reference line
                      else: 
                  endcase
              end
...
;                set_value='Expecting another s !!'
          ...
          case eventch of
              ...
              ; SMOOTH
              'S': uves_specplt_smooth, state
              'U': uves_specplt_smooth, state, /reset
              ; Crude Analysis
              'E': uves_specplt_EW, state        ; Calc EW
              'N': x_specplot_Colm, state      ; Calc AODM colm
              ' ': print, 'x: '+strtrim(state.xpos,2)+$
                '   y:'+strtrim(state.ypos,2)
              ; LINES
              'L': uves_specplt_SetLine, state   ; Set reference line
              'A': begin ; Plot AlIII
              ...
              ;; ORDERS
              '0': uves_specplt_Chgordr, state, 0
              '1': uves_specplt_Chgordr, state, 1
              '2': uves_specplt_Chgordr, state, 2
              '3': uves_specplt_Chgordr, state, 3
              '4': uves_specplt_Chgordr, state, 4
              '5': uves_specplt_Chgordr, state, 5
              '6': uves_specplt_Chgordr, state, 6
              '7': uves_specplt_Chgordr, state, 7
              '8': uves_specplt_Chgordr, state, 8
              '9': uves_specplt_Chgordr, state, 9
              '=': uves_specplt_Chgordr, state, /add
              '-': uves_specplt_Chgordr, state, /sub
              ; Postscript
              'P': uves_specplt_psfile, state  
              ; QUIT
              'q': begin
                  widget_control, ev.top, /destroy
                  return
              end
              else:  print, 'uves_specplt: Not a valid key!' ; Nothing
          endcase
      end
      'DONE' : begin
          widget_control, ev.top, /destroy
...
pro uves_specplt_UpdatePlot, state
...
; Plot Data
  ...
  if flg_lines EQ 1 then uves_specplt_PltLines, state
  
  ; EW
  if state.flg_EW NE 0 then uves_specplt_PltEW, state
...
pro uves_specplt_Reset, state

common uves_specplt_cmmn

  case state.flg_fspec of 
      0: begin ; Box
          ;; Set wave, fx
;          a = where(uvesobj[state.ordr].box_fx NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
;          amin = (a[0] > 0)
;          if amin GT 100 then amin = 0L
;          bmax = b[nb-1]
;          if bmax LT uvesobj[state.ordr].npix/2 then $
;            bmax = 4999 else bmax = a[na-1]
;          npix = bmax-amin+1
;          state.npix = npix
          amin = 0L
          bmax = uvesobj[state.ordr].nrow-1
          state.npix = bmax+1
          npix = state.npix
          
          state.wave[0:npix-1] = uvesobj[state.ordr].box_wv[amin:bmax]
          state.fx[0:npix-1] = uvesobj[state.ordr].box_fx[amin:bmax] 
          state.sig[0:npix-1] = sqrt(uvesobj[state.ordr].box_var[amin:bmax])
      end
      1: begin ; Optimal
          ;; Set wave, fx
;          a = where(uvesobj[state.ordr].wave NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
...
;          endelse
          amin = 0L
          bmax = uvesobj[state.ordr].npix - 1
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = uvesobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = uvesobj[state.ordr].fx[amin:bmax]
          state.sig[0:npix-1] = sqrt(uvesobj[state.ordr].var[amin:bmax] > 0.)
      end
      2: begin ; FSPEC
          ;; Set wave, fx
          ido = where(state.ordr EQ uvesobj.phys_ordr, na)
          if na EQ 0 then stop
          a = where(uvesobj.wave[*,ido] NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          if na EQ 0 then begin
              amin = 0L
              bmax = 100L
...
;              if amin GT 100 then amin = 0L
          ...
          state.wave[0:npix-1] = uvesobj.wave[amin:bmax,ido]
          state.fx[0:npix-1] = uvesobj.fx[amin:bmax,ido]
          state.sig[0:npix-1] = sqrt(uvesobj.var[amin:bmax,ido] > 0.)
      end
      3: begin ; fluxed
          ;; Set wave, fx
          a = where(uvesobj[state.ordr].wave NE 0., na, $
          ...
          state.wave[0:npix-1] = uvesobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = uvesobj[state.ordr].flux[amin:bmax]
          state.sig[0:npix-1] = uvesobj[state.ordr].sig[amin:bmax]
      end
      4: begin ; Sky
          ;; Set wave, fx
          a = where(uvesobj[state.ordr].sky NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          amin = (a[0] > 0)
          if amin GT 100 then amin = 0L
          bmax = b[nb-1]
          if bmax LT uvesobj[state.ordr].nrow/2 then $
            bmax = 4999 else bmax = a[na-1]
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = uvesobj[state.ordr].sky_wv[amin:bmax]
          state.fx[0:npix-1] = uvesobj[state.ordr].sky[amin:bmax] 
...
pro uves_specplt_SetLine, state
...
pro uves_specplt_PltLines, state, IMG=img
...
pro uves_specplt_Colm, state

common x_specplot_lines

  ; Check for error array
  if state.flg_sig NE 1 then begin
      print, 'uves_specplt_Colm: Need to specify the error array!'
      return
  endif
...
pro uves_specplt_guess, state, val, IMG=img
...
pro uves_specplt_smooth, state, RESET=reset
...
pro uves_specplt_Chgordr, state, newordr, ADD=add, SUB=sub
...
common uves_specplt_cmmn
  ...
  if keyword_set( ADD ) or keyword_set( SUB ) then begin
      if keyword_set( ADD ) then begin
          ...
          if newordr EQ n_elements(uvesobj) then newordr = 0
          state.ordr = newordr
      endif
      if keyword_set( SUB ) then begin
          newordr = state.ordr - 1
          if state.flg_fspec NE 2 then begin
              if newordr LT 0 then newordr = n_elements(uvesobj)-1
          endif else begin
              if uvesobj.phys_ordr[newordr] EQ 0 then begin
                  pos = where(uvesobj.phys_ordr GT 0, npos)
                  newordr = pos[npos-1]
              endif
          endelse
          state.ordr = newordr
  ...
  case state.flg_fspec of
      0: state.phys_ordr = uvesobj[state.ordr].order 
      1: state.phys_ordr = uvesobj[state.ordr].order 
      2: state.phys_ordr = uvesobj.phys_ordr[state.ordr]
      3: state.phys_ordr = uvesobj[state.ordr].order 
      4: state.phys_ordr = uvesobj[state.ordr].order 
      else: stop
  endcase

  ;; RESET
  widget_control, state.ordr_id, set_value=state.phys_ordr
  uves_specplt_Reset, state
...
pro uves_specplt, spec_fil, obj_id, XSIZE=xsize, YSIZE=ysize, $
...
;
;  print,'Syntax - ' + $
;    'uves_specplt, spec_fil, obj_id, XSIZE=,YSIZE=, /NOCTB, WAVE=, '
...
; Input the Obj structure
  if not keyword_set( FSPEC ) then begin
      if keyword_set( spec_fil ) then begin
          objstr = xmrdfits(spec_fil, 1, structyp='uvesobjstrct', /silent)
      endif else begin
          fils = findfile('Extract/Obj*fits*', count=nfil)
          if nfil EQ 0 then return 
          obj_fil = x_guilist(fils)
          objstr = xmrdfits(obj_fil, 1, structyp='uvesobjstrct', /silent)
      endelse 
      ...
      uves_wrfspec, objstr, spec_fil, /read
      gdobj = [0L]
      ;; Initial order
...
; Init common
  uves_specplt_initcommon
  uvesobj = objstr[gdobj]
  delvarx, objstr
...
; PHYSORDR
  if not keyword_set( FSPEC ) then state.phys_ordr = uvesobj[state.ordr].order $
    else state.phys_ordr = uvesobj.phys_ordr[state.ordr]
...
;    WIDGET
  base = WIDGET_BASE( title = 'uves_specplt', /column, $
...
; Send to the xmanager
  if not keyword_set(BLOCK) then xmanager, 'uves_specplt', base, /no_block $
  else xmanager, 'uves_specplt', base
