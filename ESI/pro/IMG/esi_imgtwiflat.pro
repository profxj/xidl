...
pro esi_imgtwiflat, esi, SVOV=svov, OUTROOT=outroot, ALLOUT=allout

;
  if  N_params() LT 1  then begin 
      print,'Syntax - ' + $
        'esi_imgtwiflat, esi, /SVOV, OUTROOT=, ALLOUT= (v1.1)'
      return
  endif 
  
;  Optional Keywords

  if not keyword_set( OUTROOT ) then outroot = 'Flats/FlatIMG_T'
  outroot = strtrim(outroot,2)
  
;  Find the Twilight Flats

  flat = where(esi.type EQ 'TWI' AND esi.flg_anly NE 0 $
...
;  Delete images
  ...
  print, 'esi_imgtwiflat: All done with Twilight Flats!'
