...
;
; PURPOSE:
;  Coadd and combine sky spectrum for a sky model
;
; CALLING SEQUENCE:
;   
;  esi_echskymodel, esi, obj_id, CRVAL1=, CDELT=, NPIX=, /STD
;
; INPUTS:
;   esi   -  ESI structure
...
pro esi_echskymodel, esi, obj_id, CRVAL1=crval1, CDELT=cdelt, NPIX=npix $
...
; Loop
  ...
  for qq=ordrs[0],ordrs[1] do begin
      ...
      a2 = where(spec2d.sky[*,qq] GT 0, ngd2)

      ;; Bad values
      if bad[qq,0,0] NE 0. then begin
          msk = replicate(1,ngd)
          msk2 = replicate(1,ngd2)
          ...
          a2 = a2[where(msk2 EQ 1, ngd2)]
      endif

      if qq EQ (ordrs[0]+3) then stop
         ;x_splot, spec2d.wave[a2,qq], spec2d.sky[a2,qq], /bloc, xtwo=spec2d.wave[*,qq-1], ytwo=spec2d.sky[*,qq-1]
      ;; Add em in
      ...
      stmp2   = tot_sky[indx+a2]   + double(spec2d.sky[a2, qq])
      ...
      for i = 0L, ngd2-1 do begin
           tot_sky[indx + a2[i]] = stmp2[i]
           weight_sky[indx+a2[i]] = weight_sky[indx+a2[i]] + 1.D
      endfor
  endfor
  a = where(weight NE 0.)
  tot_flux[a] = tot_flux[a] / weight[a]
  asky = where(weight_sky NE 0.)
  tot_sky[asky]  = tot_sky[asky] / weight_sky[asky]
