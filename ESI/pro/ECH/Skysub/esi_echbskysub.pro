...
pro esi_echskysub, esi, obj_id, exp, CHK=chk, STD=std, NORD=nord, ORDR=ordr, $
                   NOVAC=novac, SKLFIL=sklfil

;
  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'esi_echskysub, esi, obj_id, [exspr], /CHK, /STD, /NOVAC, ORDR=, NORD= [v1.0]'
      return
  endif 
...
;                YTWO=yfit;, XTHR=obj_w, YTHR=obj_f, PSYM_Y3=1
                YTWO=yfit, XTHR=bset.fullbkpt, YTHR=ybkpt, PSYM_Y3=2
          endif
          ...
          ;; TEST
;          obj_mn = fltarr(sz_img[1])
;          for j=0L,sz_img[1]-1 do begin
;              a = where(msk[*,j] EQ 2, na)
;              if na NE 0 then obj_mn[j] = mean(img_new[a,j])
;          endfor
;          x_splot, obj_mn, /block
;          stop
          ...
          if flg_ordr EQ 0 then begin
              ...
              spawn, 'cp '+skyfil+' tmp_bset.fits'
              case qq of 
                  0: begin
                      mwrfits, bset, skyfil, /create, /silent 
                      for i=1L,9L do begin
                          tmp_bset = xmrdfits('tmp_bset.fits', i+1, /silent)
                          mwrfits, tmp_bset, skyfil, /silent 
                      endfor
                  end
                  else: begin
                      tmp_bset = xmrdfits('tmp_bset.fits', 1, /silent)
                      mwrfits, tmp_bset, skyfil, /create, /silent 
