...
pro fig_trcobj, infil, sub

  common xcommon_color, r_vector, g_vector, b_vector
  xicmm_colors

  ;; info
  if not keyword_set(PSFIL) then psfil='fig_trcobj.ps'
  if not keyword_set(TRC_FIL) then trc_fil = $
    getenv('MIKE_PAP')+'Final/f_mr0006.fits'
  if not keyword_set(OSTRFIL) then ostrfil = $
    getenv('MIKE_PAP')+'Flats/OStr_R_01.fits'
  if not keyword_set(OBJFIL) then objfil = $
    getenv('MIKE_PAP')+'Extract/Obj_mr0006.fits'


  ordr_str = xmrdfits(ostrfil, 1, /silent)
  obj_str = xmrdfits(objfil, 1, /silent)
...
;  tv_image = congrid(display_image, ntv, ntv)
  ...
  ;; Plot
  xoff = 0.
  for q=0L, n_elements(obj_str)-1 do begin
      oplot, obj_str[q].trace, dumy, color=6, thick=2
  endfor
