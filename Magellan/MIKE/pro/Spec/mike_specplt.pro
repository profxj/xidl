...
common mike_specplt_cmmn, mikeobj
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : mike_specplt_SetLine, state   ; Set reference line
                      else: 
                  endcase
              end
...
;                set_value='Expecting another s !!'
          ...
          case eventch of
              ...
              ; SMOOTH
              'S': mike_specplt_smooth, state
              'U': mike_specplt_smooth, state, /reset
              ; Crude Analysis
              'E': mike_specplt_EW, state        ; Calc EW
              'N': x_specplot_Colm, state      ; Calc AODM colm
              ' ': print, 'x: '+strtrim(state.xpos,2)+$
                '   y:'+strtrim(state.ypos,2)
              ; LINES
              'L': mike_specplt_SetLine, state   ; Set reference line
              'A': begin ; Plot AlIII
              ...
              ;; ORDERS
              '0': mike_specplt_Chgordr, state, 0
              '1': mike_specplt_Chgordr, state, 1
              '2': mike_specplt_Chgordr, state, 2
              '3': mike_specplt_Chgordr, state, 3
              '4': mike_specplt_Chgordr, state, 4
              '5': mike_specplt_Chgordr, state, 5
              '6': mike_specplt_Chgordr, state, 6
              '7': mike_specplt_Chgordr, state, 7
              '8': mike_specplt_Chgordr, state, 8
              '9': mike_specplt_Chgordr, state, 9
              '=': mike_specplt_Chgordr, state, /add
              '-': mike_specplt_Chgordr, state, /sub
              ; Postscript
              'P': mike_specplt_psfile, state  
              ; QUIT
              'q': begin
                  widget_control, ev.top, /destroy
                  return
              end
              else:  print, 'mike_specplt: Not a valid key!' ; Nothing
          endcase
      end
      'DONE' : begin
          widget_control, ev.top, /destroy
...
pro mike_specplt_UpdatePlot, state
...
; Plot Data
  ...
  if flg_lines EQ 1 then mike_specplt_PltLines, state
  
  ; EW
  if state.flg_EW NE 0 then mike_specplt_PltEW, state
;  if state.flg_EW EQ 1 then oplot, [state.EW_lmt[0,0]], [state.EW_lmt[0,1]], $
;    psym=2, color=getcolor('red')

  ; Colm
  if state.flg_Colm NE 0 then mike_specplt_PltClm, state
...
pro mike_specplt_Reset, state

common mike_specplt_cmmn

  case state.flg_fspec of 
      0: begin ; Box
          ;; Set wave, fx
;          a = where(mikeobj[state.ordr].box_fx NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
;          amin = (a[0] > 0)
;          if amin GT 100 then amin = 0L
;          bmax = b[nb-1]
;          if bmax LT mikeobj[state.ordr].npix/2 then $
;            bmax = 4999 else bmax = a[na-1]
;          npix = bmax-amin+1
;          state.npix = npix
          amin = 0L
          bmax = mikeobj[state.ordr].nrow-1
          state.npix = bmax+1
          npix = state.npix
          
          state.wave[0:npix-1] = mikeobj[state.ordr].box_wv[amin:bmax]
          state.fx[0:npix-1] = mikeobj[state.ordr].box_fx[amin:bmax] 
          state.sig[0:npix-1] = sqrt(mikeobj[state.ordr].box_var[amin:bmax])
      end
      1: begin ; Optimal
          ;; Set wave, fx
;          a = where(mikeobj[state.ordr].wave NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
...
;          endelse
          amin = 0L
          bmax = mikeobj[state.ordr].npix - 1
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = mikeobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = mikeobj[state.ordr].fx[amin:bmax]
          state.sig[0:npix-1] = sqrt(mikeobj[state.ordr].var[amin:bmax] > 0.)
      end
      2: begin ; FSPEC
          ;; Set wave, fx
          ido = where(state.ordr EQ mikeobj.phys_ordr, na)
          if na EQ 0 then stop
          a = where(mikeobj.wave[*,ido] NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          if na EQ 0 then begin
              amin = 0L
              bmax = 100L
          ...
          state.wave[0:npix-1] = mikeobj.wave[amin:bmax,ido]
          state.fx[0:npix-1] = mikeobj.fx[amin:bmax,ido]
          state.sig[0:npix-1] = sqrt(mikeobj.var[amin:bmax,ido] > 0.)
      end
      3: begin ; fluxed
          ;; Set wave, fx
          a = where(mikeobj[state.ordr].wave NE 0., na, $
          ...
          state.wave[0:npix-1] = mikeobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = mikeobj[state.ordr].flux[amin:bmax]
          state.sig[0:npix-1] = mikeobj[state.ordr].sig[amin:bmax]
      end
      4: begin ; Sky
          ;; Set wave, fx
          a = where(mikeobj[state.ordr].sky NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          amin = (a[0] > 0)
          if amin GT 100 then amin = 0L
          bmax = b[nb-1]
          if bmax LT mikeobj[state.ordr].nrow/2 then $
            bmax = 4999 else bmax = a[na-1]
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = mikeobj[state.ordr].sky_wv[amin:bmax]
          state.fx[0:npix-1] = mikeobj[state.ordr].sky[amin:bmax] 
  ...
  ;; Plotting
  state.svxymnx = [state.wave[0], min(state.fx[0:npix-1],max=mx), $
                   state.wave[npix-1], mx]
...
pro mike_specplt_SetLine, state
...
pro mike_specplt_PltLines, state, IMG=img
...
pro mike_specplt_Colm, state

common x_specplot_lines

  ; Check for error array
  if state.flg_sig NE 1 then begin
      print, 'mike_specplt_Colm: Need to specify the error array!'
      return
  endif
...
pro mike_specplt_guess, state, val, IMG=img
...
pro mike_specplt_smooth, state, RESET=reset
...
pro mike_specplt_Chgordr, state, newordr, ADD=add, SUB=sub
...
common mike_specplt_cmmn
  ...
  if keyword_set( ADD ) or keyword_set( SUB ) then begin
      if keyword_set( ADD ) then begin
          ...
          if newordr EQ n_elements(mikeobj) then newordr = 0
          state.ordr = newordr
      endif
      if keyword_set( SUB ) then begin
          newordr = state.ordr - 1
          if state.flg_fspec NE 2 then begin
              if newordr LT 0 then newordr = n_elements(mikeobj)-1
          endif else begin
              if mikeobj.phys_ordr[newordr] EQ 0 then begin
                  pos = where(mikeobj.phys_ordr GT 0, npos)
                  newordr = pos[npos-1]
              endif
          endelse
          state.ordr = newordr
  ...
  case state.flg_fspec of
      0: state.phys_ordr = mikeobj[state.ordr].order 
      1: state.phys_ordr = mikeobj[state.ordr].order 
      2: state.phys_ordr = mikeobj.phys_ordr[state.ordr]
      3: state.phys_ordr = mikeobj[state.ordr].order 
      4: state.phys_ordr = mikeobj[state.ordr].order 
      else: stop
  endcase

  ;; RESET
  widget_control, state.ordr_id, set_value=state.phys_ordr
  mike_specplt_Reset, state
...
;
;  print,'Syntax - ' + $
;    'mike_specplt, spec_fil, obj_id, XSIZE=,YSIZE=, /NOCTB, WAVE=, '
...
; Input the Obj structure
  if not keyword_set( FSPEC ) then begin
      ...
      mike_wrfspec, objstr, spec_fil, /read
      gdobj = [0L]
      ;; Initial order
...
; Init common
  mike_specplt_initcommon
  mikeobj = objstr[gdobj]
  delvarx, objstr
...
; PHYSORDR
  if not keyword_set( FSPEC ) then state.phys_ordr = mikeobj[state.ordr].order $
    else state.phys_ordr = mikeobj.phys_ordr[state.ordr]
...
; LINELIST
  ...
  if keyword_set( LLS ) then state.flg_lines = 6
  if state.flg_lines NE 0 then x_specplot_initLines, state
  
;    WIDGET
  base = WIDGET_BASE( title = 'mike_specplt', /column, $
...
; Send to the xmanager
  if not keyword_set(BLOCK) then xmanager, 'mike_specplt', base, /no_block $
  else xmanager, 'mike_specplt', base
