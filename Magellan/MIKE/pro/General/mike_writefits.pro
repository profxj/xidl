...
pro mike_writefits, mike, indx, SILENT=silent, OUTDIR=outdir, $
  ...
  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'mike_writefits, mike, indx, /silent, /ASCII, /PHEAD, /AFLUX (v1.0)'
      return
  endif 
...
;
  ...
  if not keyword_set(FSPEC) then begin
      for qq=0L,nindx-1 do begin
          ;;  
          idx = indx[qq]
          objfil = mike_getfil('obj_fil', mike[idx].setup, $
                               SUBFIL=mike[idx].img_root, /name)
;          if chkf EQ 0 then continue
          ;; Write
          obj = xmrdfits(objfil, 1, /silent)
          
          ;; Header
          finfil = mike_getfil('fin_fil', mike[idx].setup, $
                               SUBFIL=mike[idx].img_root, /name)
          ...
          while( DUM LT 1000 ) do begin
              ...
              cframe = cframe+'0'
          endwhile
          cframe = cframe+strtrim(mike[idx].frame,2)
          ...
          for ii=0L,nordr-1 do begin
              ...
              if keyword_set( FITS) then begin
                  ;; Wavelengths
                  outwv = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'_wv.fits'
                  mwrfits, obj[ii].wave[0:obj[ii].npix-1], outwv, head, /create
                  
                  ;; Flux
                  outfx = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'_fx.fits'
                  mwrfits, obj[ii].fx[0:obj[ii].npix-1], outfx, head, /create
                  
                  ;; Sigma
                  outer = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'_er.fits'
                  mwrfits, sqrt(obj[ii].var[0:obj[ii].npix-1]>0), $
                           outer, head, /create
              endif
              if keyword_set( ASCII ) then begin

                  outfil = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'.asc'
              ...
              if keyword_set( AFLUX ) then begin
                  outfil = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'.afx'
                  writecol, outfil, obj[ii].wave[0:obj[ii].npix-1], $
          ...
          if keyword_set( PHEAD ) then begin
              outfil = outdir+mike[idx].obj+'_'+cframe+'_'+cc+'.hdr'
      ...
      for qq=0L,nindx-1 do begin
          ;;  
          idx = indx[qq]
          subfil = strcompress(strtrim(mike[idx].Obj,2),/remove_all)+obj_nm
          case mike[idx].chip of 
              1: clrc = '_B' 
              2: clrc = '_G'
              3: clrc = '_R'
          endcase
          ...
          ;; Header
          finfil = mike_getfil('fin_fil', /name, $
                                FRAME=mike[indx[qq]].frame,$
                                CHIP=mike[indx[qq]].chip, chkfil=chkf)
          head = xheadfits(finfil)
          sxaddpar, head, 'CRPIX1', 1
          sxaddpar, head, 'CTYPE1', 'LINEAR'
          sxaddpar, head, 'DC-FLAG', 1
          ...
          for jj=0L,nordr-1 do begin
              ...
              if keyword_set( FITS) then begin
                  ...
                  velpix = 1.3d * mike[idx].rowbin
                  cdelt1 = alog10(1.0d + velpix / 299792.458d)
                  sxaddpar, head, 'CRVAL1', crval1
                  sxaddpar, head, 'CDELT1', cdelt1
                  
                  ;; Flux
                  outfx = outdir+mike[idx].obj+'_'+cc+co+'_fx.fits'
                  mwrfits, obj.fx[gdp,ii], outfx, head, /create
                  
                  ;; Var
                  outer = outdir+mike[idx].obj+'_'+cc+co+'_var.fits'
                  mwrfits, obj.var[gdp,ii], outer, head, /create
              endif
              if keyword_set( ASCII ) then begin
                  IF NOT KEYWORD_SET(cframe) THEN $
                    outfil = outdir+mike[idx].obj+'_'+cc+co+'.asc' $
                  ELSE outfil = outdir+mike[idx].obj+'_'+cframe+'_'+cc+co+'.asc'
                  writecol, outfil, obj[ii].wave[0:obj[ii].npix-1], $
                            obj[ii].fx[0:obj[ii].npix-1], $
                            obj[ii].var[0:obj[ii].npix-1], $
                            FMT='(f10.4,1x,f14.4,1x,f10.4)'
