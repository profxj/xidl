...
;
; EXAMPLES:
;   fit = mage_echfitstd(x, y, 'POLY', nord=5, /inter)
;
;
...
pro mage_echfitstd_initcommon, inord, fitstr, NFITORD = NFITORD, FUNC = FUNC

;

common mage_echfitstd_fit, nord, tot_fit, sens_func

  nord = inord
  IF NOT KEYWORD_SET(NFITORD) THEN NFITORD = 15L
  IF NOT KEYWORD_SET(FUNC) THEN FUNC = 'LEGEND'
  if not keyword_set( FITSTR ) then $
     fitstr = x_setfitstrct(FUNC = FUNC, NORD = NFITORD $
                            , LSIG = 3., HSIG = 3., FLGREJ = 1)
  if n_elements(fitstr) ne nord then $ 
...
;function mage_echfitstd_ev, ev
pro mage_echfitstd_event, ev

common mage_echfitstd_fit 
  ...
  case uval of
      ...
      'FUNCLIST' : begin
          ...
          ; Update fit
          mage_echfitstd_UpdateFit, state
      end
      'UP' : begin
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : mage_echfitstd_setordr, state
                      else :
                  endcase
              end
      ...
      'TEXT' : begin
          ...
          if (state.flg_zero EQ 1 AND eventch NE 's') then begin
              print, 'mage_echfitstd: Expecting another s or LMB click !!'
              WIDGET_CONTROL, state.base_id, set_uvalue = state, /no_copy
              return
           endif
           if (state.flg_unzero EQ 1 AND eventch NE 'x') then begin
              print, 'mage_echfitstd: Expecting another x'
              WIDGET_CONTROL, state.base_id, set_uvalue = state, /no_copy
              return
           endif
          case eventch of
              'u': begin
                  ...
                  print, 'mage_echfitstd: Fit order = ', tot_fit[gdo].nord
;                  widget_control, state.lblordr_id, $
;                    set_value=string(fitprm.nord, format='(i4)')
                  mage_echfitstd_UpdateFit, state, ordr=state.currordr
              end
              'd': begin
                  if state.currordr LT 1 then return
                  gdo = where(state.objstr.order EQ state.currordr)
...
;              end
              ...
              'x': begin        ; Region
                  mage_echfitstd_unzeroreg, state
...
;              'D': mage_echfitstd_Delete, state ; Delete/Undelete a data point
;              'x': mage_echfitstd_DelReg, state        ; Delete one region
;              'S': mage_echfitstd_DelReg, state, /all  ; Delete all regions and reset
...
;              'V': mage_echfitstd_SetValues, state     ; Plot values
;              'C': mage_echfitstd_ClearRej, state ; Clear all rejected points
              'H': x_helpwidg, state.help
              'q': begin
;                  mage_echfitstd_setpnt, state
...
; Update Plot
  mage_echfitstd_UpdatePlot, state
...
pro mage_echfitstd_UpdatePlot, state, ALL=all
...
pro mage_echfitstd_UpdateFit, state, ALL=all, ORDR=ordr
...
;  Set order
;;;;;;;;;;;;;;;;;;;;
pro mage_echfitstd_setordr, state

  ;; Grab x position
  xv = xgetx_plt(state,/strct)

  ;;gdw = where(state.objstr.wave GT state.xymnx[0] AND $
  ;;            state.objstr.wave LT state.xymnx[2] AND $
  ;;            state.objstr.sig GT 0, ngdw)
  ;; Grab closest pixel
  ;;mn = min(abs(state[gdw].objstr.wave-xv), imn)
  ;;dwv = abs((state.objstr.wave)[gdw[imn]] - (state.objstr.wave)[gdw[imn+1]])

  ;; Grab closest within 10pixels
  ;;gd = where(abs(state.objstr.wave[gdw]-xv) LT 10*dwv)
  ;;mx = max((state.objstr.flux)[gdw[gd]], imx)
  ...
  ;; Order
  ;;state.currordr =  (state.ordnum)[gdw[gd[imx]]]
...
pro mage_echfitstd_Reset, state
  ...
  ;; Good ones
  gd = where(state.objstr.wave GT 0. and state.svsig GT 0. )
...
pro mage_echfitstd_zeroreg, state

  ;; Check current order
  if state.currordr LT 1 then begin
      print, 'mage_echfitstd:  Need to set the order with RMB first!'
      return
  endif
...
pro mage_echfitstd_unzeroreg, state
...
; This routine added by JFH to unzero regions  
pro mage_echfitstd_forcereg, state
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;  Reset SVXY
pro mage_echfitstd_UpdateSVXY, state
  ; Set xdat, ydat
...
pro mage_echfitstd, objstr, std_wv, std_fx, outfil, XSIZE=xsize, FITSTR=fitstr $
                    , INFIL = infil, savefil = savefil, insave = insave $
                    , NFITORD = NFITORD, FUNC = FUNC
...
;
  if  N_params() LT 4  then begin 
    print,'Syntax - ' + $
      'fit = mage_echfitstd(objstr, std_wv, std_fx, outfil, ' + $
      'FITSTR=,XSIZE=, INFIL=) [v1.1]'
    return
  endif 

  IF KEYWORD_SET(INSAVE) THEN begin
     restore, insave
     fitstr = tot_fit
  endif
...
;  readcol, stdfil, swv, sfx
  ...
  mage_echfitstd_initcommon, n_elements(objstr), fitstr, NFITORD = NFITORD $
                             , FUNC = FUNC
  sens_func = objstr.fx
  sens_func[*] = 0.
  ...
  ;;    WIDGET
  base = WIDGET_BASE( title = 'mage_echfitstd: Interactive ' + $
...
; Realize
  ...
  ;; Update
  mage_echfitstd_Reset, state
  mage_echfitstd_UpdateFit, state, /all
  mage_echfitstd_UpdatePlot, state, /all
  WIDGET_CONTROL, base, set_uvalue = state, /no_copy
  
  xmanager, 'mage_echfitstd', base
