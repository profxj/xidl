...
;device, /isolatin1
  ...
  plot, [wv], [(fit-wv)/dwv], psym=7, $
       ...
       ; xmargin=[11,1], ymargin=[5,1], 
        yrange=[-0.7, 0.7], xrange=[mn-1,mx+1.], $
  ...
  xyouts, xlbl, 0.48, 'RMS (pix)='+string(rms, FORMAT='(f6.3)'), charsize=1.3
  xyouts, xlbl, 0.36, 'Fit Ord='+string(fordr, FORMAT='(i2)'), charsize=1.3
...
;     linlist = getenv('MAGE_DIR')+'/Calib/mike_thar.lst' 
  if not keyword_set( PKWDTH ) then pkwdth = 4.0  
;=======
  if not keyword_set( LINLIST ) then message,'Must specifiy linelist'
;>>>>>>> 1.4
  ;;if not keyword_set( NOLOG ) then FLOG = 1 else FLOG = 0
  if not keyword_set( SATUR ) then satur = 5d4
...
;;                         FITSTR=tmp_fit, REJPT=rejpt)
          ...
          if sv_lines[ii].nlin GT 0 then begin
             ...
             print, ', RMS (Ang,Pix) = ' ,fin_fit.rms*dwv, format='(a,f7.4,$)'
