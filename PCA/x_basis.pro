...
; REVISION HISTORY:
;   ??-2003 Written by SMB
;   ??-2004 Modified by JXP
;-
;------------------------------------------------------------------------------

function x_basis, tset, xcen, outmask, ncoeff=ncoeff, eigenvec=eigenvec, $
                  outstr=outstr, msktrc=msktrc, skipx0=skipx0, $
                  poly_ncoeff=poly_ncoeff, x0in=x0in, NONLIN=nonlin, $
                  n_xpfit=n_xpfit

  if  N_params() LT 3  then begin 
      print,'Syntax - ' + $
        'fit = x_basis(tset, xcen, outmask, NCOEFF=, EIGENVEC=,' + $
        'OUTSTR=, MSKTRC=) [v1.1]'
      return, -1
  endif 

  if not keyword_set(n_xpfit) then n_xpfit = 3
  ...
  ;; PCA analysis
  x_pca, tset.coeff[1:ncoeff-1,usetrace], ncoeff-1, eigencoeff, hidden
  ...
  ;; Linear fits to first two PCA coefficients
  if NOT keyword_set(poly_ncoeff) then $
    poly_ncoeff = (long(3.3 * ngoodtrace / ntrace ) > 1 ) < 3
  print, 'x_basis: fitting first order with polynomial order ', $
    poly_ncoeff
  ...
  for jj=0,ngoodtrace-5 do begin 
      u = where(mask1) 
      if not keyword_set( NONLIN ) then begin
          coeff1 = ladfit(x0in[usetrace[u]], y[u]) 
          fit1 = poly(x0in[usetrace], coeff1) 
      endif else begin
          coeff1 = func_fit(x0in[usetrace], y, 2, $
                            function_name='poly', invvar = mask1, yfit=fit1) 
      endelse
      ;; reject
...
;    mask = (abs(denom) GT 10 AND msk EQ 1B)
      for ii = 1,2 do begin 
          ...
          x0res = poly_fit(x0in[good], x0[good], n_xpfit)
          x0fit = poly(x0in, x0res)
          chi2 = (x0 - x0fit)^2*mask
          mask = mask * (chi2 LT total(chi2)/2.)
          reduced_chi2 =  total(chi2)/total(mask)
          print, 'x_basis: reduced_chi2 --', reduced_chi2
          ;stop
      endfor
      if reduced_chi2 GT 2.0 then $
        print, 'X_BASIS: Very large residuals, check traces' $
      else if reduced_chi2 GT 0.5 then $
        print, 'X_BASIS: Large residuals, check traces' 
