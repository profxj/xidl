...
function eval_splinefn, splinefn_strct, lgNval, z, VEL_ARRAY=vel_array, $
  ...
  lgfNX = x_spline_pchip_val(fnstr.npivot, $
                             fnstr.pivots[1:fnstr.npivot],$
                             fnstr.fn_pivot[1:fnstr.npivot], $
                             fnstr.splin[1:fnstr.npivot], $
                             n_nval, lgNval)

  ;; Find the right redshift evolution
  ...
  if keyword_set(VEL_ARRAY) then begin
     ...
     ;; 
     lgNHI_grid = lgfNX # replicate(1., n_elements(z_val))
     ;; 
