...
pro x_echcombspec, allobj, finspec, ordrs, keyindx, SILENT=silent, $
                    SNRMIN=snrmin, MEDINDX=medindx, $
                    NOFLUX=noflux, FCHK=fchk, BOXCAR=BOXCAR, $
...
;          if nobj NE nspec then stop
         ;; BOXCAR extraction
          if keyword_set( BOXCAR ) then begin
             if qq EQ ordrs[0] then print, 'x_echcombspec: Coadding with BoxCar'
             npix = allobj[all_ordr[kk]].npix
             if npix GT 0 then begin
                allobj[all_ordr[kk]].wave[0:npix-1] = $
                   allobj[all_ordr[kk]].box_wv[0:npix-1] 
                allobj[all_ordr[kk]].flux[0:npix-1] = $
                   allobj[all_ordr[kk]].box_fx[0:npix-1] 
                allobj[all_ordr[kk]].sig[0:npix-1] = $
                   sqrt(allobj[all_ordr[kk]].box_var[0:npix-1])
                allobj[all_ordr[kk]].nosig[0:npix-1] = $  ;; Not quite right
...
;          else stop 
  ...
  ;; What is the point of the following code?  [JXP: November 2011]
...
;          all_ordr = where(allobj.order EQ qq, nobj)
      ...
      ;; Get Median, sig
      gdo = where(svmedian[kk,ordrs[0]:ordrs[1]] GT 0.,ngdo)
      ;; bug with median function if only 1 image added this next line (MN)
      if ngdo eq 0 then medianave[kk]=svmedian[kk,ordrs[0]] else $
      medianave[kk] = median(svmedian[kk,ordrs[0]+gdo])
      if ngdo GT 1 then begin
         for qq=ordrs[0],ordrs[1],ostep do begin
            if svmedian[kk,qq] GT 0. then $
               mediansig[kk] = mediansig[kk] + (medianave[kk]-svmedian[kk,qq])^2.
         endfor
         mediansig[kk] = sqrt(mediansig[kk]/ngdo)
      endif else mediansig[kk] = 1.
      ;; Check for outliers
...
;          all_ordr = where(allobj.order EQ qq, nobj)
          ...
          ;if qq EQ 84 and kk GT 0 then stop
...
;          if nobj NE nspec then continue
          ...
          if ordr_typ[kk,qq] EQ 1 OR ordr_typ[kk,qq] EQ 0 then begin
              ...
              if keyword_set(CHK) and kk EQ (nexp-1) then begin
                wuse = where(allobj[all_ordr[keyindx]].wave NE 0.)
                ptit = 'Rescaled 0, 1, Order: ' + strtrim(string(qq),2) + $
                       ' and Exposure (red): ' + strtrim(string(kk),2)
                x_splot, allobj[all_ordr[keyindx]].flux[wuse], TITLE=ptit,$
...
;          gdp = where(nvar GT 0. and finspec.wave GT 0.,ngdp)
      ...
      if keyword_set( FCHK ) then begin
          wuse = where(finspec.wave[*,qq] NE 0.)
          x_splot, finspec.wave[wuse,qq], finspec.fx[wuse,qq], $
             TITLE='Combined -- Order: '+strtrim(qq,2), /block
          ;  ytwo=allobj[all_ordr[0]].flux[wuse], TITLE='Combined', $
          ;  ythr=allobj[all_ordr[1]].flux[wuse], psym3=-3, /block
      endif
  endfor
