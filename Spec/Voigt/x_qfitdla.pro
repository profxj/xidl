...
; CALLING SEQUENCE:
;   
;   x_qfitdla, ydat, [head], XSIZE=, YSIZE=, TITLE=, WAVE=
;
; INPUTS:
;   ydat       - Values 
;   [head]     - Header
...
; Update Plot
  if flg_plt EQ 1 then xqfitdla_UpdatePlot, state
...
pro x_qfitdla_inifit, state, inifit


  ;; File?
  a = findfile(inifit, count=na)
  if na EQ 0 then begin
      print, 'x_qfitdla: FILE ', inifit, ' does not exist!'
      stop
  endif
...
pro x_qfitdla, yin, ysin, XSIZE=xsize, YSIZE=ysize, TITLE=title, $
...
;
  if  N_params() LT 1  then begin 
    print,'Syntax - ' + $
             'x_qfitdla, fx, ys_in XSIZE=,YSIZE=, TITLE=, WAVE=, '
    print, '            INFLG=, /BLOCK) [v1.1]'
    return
  endif 
...
;    WIDGET
  base = WIDGET_BASE( title = 'x_qfitdla', /column)
  state.base_id = base
...
; HELP
  state.help[0] = '  :::Help Menu for x_qfitdla::: '
  ...
  state.help[19] = 'I -- IDL output'

; Update
  xqfitdla_Reset, state

  ;; Init Fit
  if keyword_set(INIFIT) then x_qfitdla_inifit, state, inifit
  ;; Set pmnx
  state.xpmnx = x_getxpmnx(state)
  ;; Plot
  xqfitdla_UpdatePlot, state
  WIDGET_CONTROL, base, set_uvalue = state, /no_copy

  print, 'Press H for help'

; Send to the xmanager
  if not keyword_set(BLOCK) then xmanager, 'x_qfitdla', base, /no_block $
  else xmanager, 'x_qfitdla', base
