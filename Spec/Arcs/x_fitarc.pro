...
function x_fitarc, arc_fil, ordr_str, out_fil, $
                   ...
                   IROW_SHIFT=irow_shift, PKWDTH=pkwdth, TOLER=TOLER, $
...
;
  ...
  if not keyword_set( TOLER ) THEN TOLER = 2.0
...
;              npix = n_elements(sv_aspec[*,ii])
             ...
             ;if jj EQ 135 then debug2 = 1 else debug2 = 0
              x_templarc, sv_aspec[*,ii], lines, guess_fit[igss], MSK=msk, $
                          ...
                          toler=toler,STRETCH=stretch, DEBUG=debug2
              gdfit = where(lines.flg_plt EQ 1, ngd)
              ;;; JFH TESTING HACK!!!!! 
              ;;IF ngd LE 10 OR flg_templ EQ -1 THEN BEGIN
                 ;;print,"JFH HACK NEEDS TO BE REMOVED"
                 ;;psig=10.0
                 ;;toler=4.0
                 ;;x_templarc, sv_aspec[*,ii], lines, guess_fit[igss], MSK=msk, $
                   ;;          SHFT=shft, ALL_PK=all_pk, PKWDTH=pkwdth, FORDR=9, LOG=FLOG, $
