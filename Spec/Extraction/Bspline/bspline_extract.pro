...
function bspline_workit, xdata, ydata, invvar, action, sset, $
   ...
   if (arg_present(yfit)) then $
     yfit = bspline_valu(xdata, sset, x2=xdata, action=action, $
   ...
   while (((error[0] NE 0) OR (keyword_set(qdone) EQ 0)) $
    AND iiter LE maxiter) do begin
      ...
      if (ngood LE 1 OR goodbk[0] EQ -1) then begin
        ...
        ;    0 if fit is good
        ;   -1 if some break points are masked ;; JXP
        ;   -2 if everything is screwed

        ;  we'll do the fit right here..............


          bf1 = bspline_action(1.0d*xdata, sset, lower=loweraction, $
                               upper=upperaction)
        ...
        ; help, where(finite(bf1) EQ 0), where(finite(action) EQ 0)
          ...
          if total(finite(action) EQ 0) GT 0 then begin
             print, '!! Infinities in action matrix, wavelengths ' + $
                    'may be very messed up!!!'
             return, 0
          endif
          
          error = bspline_workit(xdata, ydata, invvar*maskwork, $
      ...
      if (error[0] LT -1L) OR (nbkpt EQ 0) then begin
          ;; All break points have been dropped.
          print, 'bspline_extract: All break points have been dropped!!'
          stop
          return, sset
...
;            total(maskwork EQ 0), replicate(string(8b),16)
      endif else if (error[0] EQ -1) then begin ;; JXP Kludge on April 23, 2012
         print, 'bspline_extract: Some breakpoints have been dropped. Iterating'
      endif
