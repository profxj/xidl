...
function x_fntobj_off, ordr_str, img, qq, DEBUG=debug, xerr=xerr, $
                          CHK=chk
...
pro x_fntobj, objstr, ordr_str, img, ivar, qafil, $
...
;  if not keyword_set(OBJAPER) then objaper = [10.,10.]
  ...
  frac_image = fltarr(nordr)
  xerr_array = fltarr(nordr)
  for qq = 0,nordr-1 do begin  
     ;print, 'x_fntobj:  qq= ', qq, ordr_str[qq].order
     ;if qq EQ 27 then stop
      frac_array[qq] = x_fntobj_off(ordr_str, img, qq, xerr=xerr_temp, $
                                    debug=debug, CHK=chk) 
      xerr_array[qq] = xerr_temp 
      ;; Fraction of order on the slit
      gdi = where(ordr_str[qq].lhedg GT 0 and $
                  ordr_str[qq].rhedg LT sz_img[0], ngdi)
      frac_image[qq] = float(ngdi) / float(sz_img[1])
  endfor
  good = where(xerr_array GT 0 AND xerr_array LT 0.4 $
               AND frac_image GT 0.8, ngood)
...
;
  if x3fit[0] NE -1L then begin
      ...
      ;; Require the trace falls within the slit!
      for kk=0L,nordr-1 do begin
          bad = where(x4[*,kk] LT ordr_str[kk].lhedg[*] OR $
                      x4[*,kk] GT ordr_str[kk].rhedg[*], nbad)
          if nbad NE 0 then x_ivar[bad,kk] = 0
      endfor
...
;          print, objstr[mm].xcen, objstr[mm].ycen
      ...
      if nfullfit NE 0 then begin
          xy2traceset, yset, x4, tset4full, $;invvar=x_ivar, $
  ...
  ;; Demand the object remain in the slit!
  xfinal_fit = (ordr_str.lhedg-5) > xfinal_fit < (ordr_str.rhedg+5)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;      
  ...
  if keyword_set(QAFIL) then begin
      ...
      if frac_good[0] NE -1 then oplot, [frac_good], [frac_array[frac_good]],ps=1 
      oplot, frac_guess
