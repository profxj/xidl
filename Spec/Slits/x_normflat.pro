...
;   15-Feb-2002 Revsied to allow normalization of original
;-
;------------------------------------------------------------------------------
...
; Sort to require descending order
  ...
  if not keyword_set( ORIG ) then begin
      ...
      for i=0L,nslit-1 do begin
                                ; TOP
          if i GT 0 then $
            med_yedg[1,i] = (round(srt_yedg[1,i])-ymed) < round(srt_yedg[0,i-1]) $
          else $
            med_yedg[1,i] = round(srt_yedg[1,i]) - ymed
      ; BOTTOM
          if i LT nslit-1 then $
            med_yedg[0,i] = (round(srt_yedg[0,i])+ymed) > round(srt_yedg[1,i+1]) $
          else $
