...
;                be a flat-field image, an arc image, or a sum of
;                those
;;  ivar    -    Inverse variance of this image
...
;   SLIT_EDG_TWK - runs an extra step to improve determination of slit
...
;------------------------------------------------------------------------------
pro long_slitmask_work, image, ivar, hdr, outfile, biasfile = biasfile $
                   , xstart = xstart1, xend = xend1, nslit = nslit $
...
;; default behavior
if not keyword_set(NO_HEFIXES) then HEFIXES = 1L
;HEFIXES=0L
...
ELSE IF strmatch(instrument, '*ISIS*') THEN BEGIN
   ;;check windowing enable
   winenab  = strtrim(sxpar(hdr[*,0], 'WINSEC1'), 2)
   IF strmatch(winenab, '*disabled*') THEN  nowin = 1 ELSE nowin=0
   tset_slits = isis_slitset(nx, ny,nowin=nowin)
ENDIF ELSE IF strmatch(instrument, 'MMT Blue*') THEN $
   tset_slits = mmt_slitset(nx, ny, CHANNEL = 'BLUE') $
...
;
     ...
     ; peakval = 1
      ...
      allx = allx[isort]
      allq = allq[isort]
      ;;JOSH TEST TO MAKE SURE NO SLITS ARE MISSED:
   ...
   if keyword_set(EDIT_SEDGE_FIL) then begin
      ...
      for kk=0L,nslit-1 do begin
         ...
         if ctmtch GE 1 AND ctmtch LE 2 then begin
            ...
            if ctmtch EQ 2 then begin
               slitsec = where(findgen(ny) ge hand_lower[mtchbadslit[1]] and findgen(ny) $
                               le hand_upper[mtchbadslit[1]])
               if hand_flg[mtchbadslit[1]] eq 1L then begin         
                  tmp_xpos1[slitsec] = tmp_xpos1[slitsec]+hand_npix[mtchbadslit[1]]
                  print, 'long_slitmask: editing left edge of slit at xpos = ', tmp_xposavg
               endif else begin
                  if hand_flg[mtchbadslit[1]] eq 2L then begin     
                     tmp_xpos2[slitsec] = tmp_xpos2[slitsec]-hand_npix[mtchbadslit[1]]
                     print, 'long_slitmask: editing right edge of slit at xpos = ', tmp_xposavg
                  endif else begin
                     print, 'long_slitmask: Bad slit side specification (should be 1 or 2)'
                     stop
         ...
         ENDIF ELSE IF ctmtch EQ 0L THEN BEGIN
            print, 'You dont seem to want to change the slit at xpos = ', tmp_xposavg
                                ;stop
         ENDIF ELSE BEGIN 
            print, 'long_slitmask: Ambiguity in slit selection-- you may want to lower mtchtol'
         ENDELSE
      endfor 
   endif 
;stop
