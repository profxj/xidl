...
FUNCTION niri_superdark, filenames, sigrej = sigrej, maxiter = maxiter $
                         ...
                         , TSPEC = TSPEC, ISAAC = ISAAC, LUCI = LUCI $
                         , SOFI = SOFI $
                         , SINFONI = SINFONI, OUTFILE = OUTFILE
...
;     Read this file
    ...
    ELSE IF KEYWORD_SET(TSPEC) THEN $
       tspec_proc, filenames[ifile], image1, hdr = hdr1 $
                   , verbose = verbose $
    ELSE IF KEYWORD_SET(LUCI) THEN $
       luci_proc, filenames[ifile], image1, hdr = hdr1 $
                  , verbose = verbose $
    ELSE IF KEYWORD_SET(SINFONI) THEN $
       sinfoni_proc, filenames[ifile], image1, hdr = hdr1 $
                     , verbose = verbose $
    ELSE IF KEYWORD_SET(SOFI) THEN $
       sofi_proc, filenames[ifile], image1, hdr = hdr1 $
                  , verbose = verbose $
...
;          inmask = make_array(dimension = [dims, nfile], /byte)
        sxaddpar, hdr, 'STACKED', 1, 'Combined dark from niri_superdark.pro', $
                  before = 'EXPTIME'
...
;                             inmask = inmask) $
ELSE imgfinal = image1
; Write output file
IF KEYWORD_SET(OUTFILE) THEN  mwrfits, imgfinal, outfile, hdr, /create
