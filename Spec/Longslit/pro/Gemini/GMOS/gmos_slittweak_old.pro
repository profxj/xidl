FUNCTION gmos_slittweak, flat, tset_slits_in $
                         , MAXSLIT = MAXSLIT, MINSLIT = MINSLIT $
                         , CHK = CHK 
...
;    normspat = smooth(flat_nrm[ipix[psort]], med_spat_width, /edge_trunc)
      ...
      IF KEYWORD_SET(CHK) THEN BEGIN
          splot, frac[psort], flat_nrm[ipix[psort]], psym = 3 $
                 , xr = [min(frac[psort]), max(frac[psort])], yr = [0.0, 2.0] $
                 , title = 'slit # ' + strcompress(string(islit+1L), /rem)
          soplot, frac[psort], normspat, col = 1, psym = 4
          IF KEYWORD_SET(tweak_left) THEN $
             soplot, [xleft, xleft], [0.0, 2.0], linestyle = 2, col = 2
          IF KEYWORd_SET(tweak_right) THEN $
             soplot, [xright, xright], [0.0, 2.0], linestyle = 2, col = 2
          stop
;;          wait, 1
