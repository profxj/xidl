PRO GMOS_IMAG_REDUCE, biasfiles, darkfiles, flatfiles, datafiles $
                      , RAWPATH = RAWPATH, IRAFPATH = IRAFPATH

  IF NOT KEYWORD_SET(RAWPATH) THEN RAWPATH = 'Raw/'
  IF NOT KEYWORD_SET(IRAFPATH) THEN IRAFPATH = 'IRAF/'

  procfiles = 'proc_imag-' + datafiles
  ivarfiles = 'proc_ivar-' + datafiles
  
;; Create a superflat
  superflatfile = 'superflat-' + flatfiles[0] 
  gmos_imag_superflat, rawpath + flatfiles,  superflatfile 
  
;;; create the superdark and the darkmask
  superbiasfile = 'superbias-' + biasfiles[0] 
  superdarkfile = 'superdark-' + darkfiles[0] 
  gmos_superdark, rawpath + biasfiles, rawpath + darkfiles, superbiasfile $
                  , superdarkfile, /IMAGING
...
;;; For a single file, run this
     ...
     IF ii EQ 0 THEN BEGIN
        ...
        dark_dum2 = dark_dum1
        dark_dum3 = dark_dum1
        ;; Chip1 and Chip2 have oscan on right side
        ;; Chip 3 has oscan on left side
