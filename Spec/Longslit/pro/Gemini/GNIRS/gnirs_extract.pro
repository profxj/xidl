...
FUNCTION GNIRS_EXTRACT, img_minsky, sciivar, waveimg, thismask, sky_model $
                        ...
                        , KLUDGE_TELLURIC = KLUDGE_TELLURIC $
                        , TELLURIC = TELLURIC $
...
IF KEYWORD_SET(KLUDGE_TELLURIC) THEN ERRSCL = 0.01 ELSE ERRSCL = 1.0
;; Maximum FWHM is 1.3"
...
if NOT keyword_set(box_AP) then box_ap = 2.5
IF KEYWORD_SET(TELLURIC) THEN BOX_AP = 5.0D
box_rad = box_ap/plate_Scale
...
IF median(wave) GT 5000.0 THEN units = 1d4 $
ELSE units = 1.0d
...
IF ARG_PRESENT(IMG_PROFILE) THEN BEGIN
   IF nobj GT 1 THEN message, 'Problem with image profile'
   dim = size(img_profile, /dim)
   IF dim[0] NE nx OR dim[1] NE ny THEN message, 'Problem with image profile'
   img_profile1 = fltarr(nx, ny)
   img_profile1[ipix] = obj_profiles
   ;; only fill this slit with the profile
   ithis = WHERE(thismask)
   img_profile[ithis] = img_profile1[ithis]
ENDIF
