...
;-
;------------------------------------------------------------------------------
pro luci_slitmask, filename, outfile, darkfile = darkfile $
                   , xstart = xstart1, xend = xend1 $
                   , minslit = minslit, y1 = y1_in, y2 = y2_in, nmed = nmed $
                   , ksize = ksize, peakthresh = peakthresh1 $
                   ...
                   , SLIT_EDG_TWK = SLIT_EDG_TWK, SOFI = SOFI
...
; Read raw image
  ...
  IF KEYWORD_SET(SOFI) THEN sofi_proc, filename, image, ivar $
                                       , hdr = hdr, darkfile = darkfile, verbose = verbose $
  ELSE luci_proc, filename, image, ivar, hdr = hdr, darkfile = darkfile, verbose = verbose 

  IF KEYWORD_SET(peakthresh1) THEN peakthresh = peakthresh1 ELSE peakthresh = 0.3
