FUNCTION LUCI_WAVEIMG, arcimg, arcivar, tset_slits, hdr $
                       , scifile = scifile $
                       , QAFILE = QAFILE $
                       , piximg = piximg, MXSHIFT = MXSHIFT $
                       , SIGREJ = SIGREJ, WAVEFILE = WAVEFILE $
                       , BOX_RAD = BOX_RAD, CHK = CHK $
                       , CALIB = CALIB, pixset = pixset, NOSAVE = NOSAVE
...
arc_ncoeff = 7 ;; ???more curvature in arc lines use higher order?????
...
savefile = 0 ;; Don't write out solutions
