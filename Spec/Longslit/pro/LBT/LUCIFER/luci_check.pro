...
print, file_fits
this = findfile(psfile, count = nps)
IF nps GT 0 THEN spawn, 'gv  ' + psfile + '&'

diff = mrdfits(file_fits, 0, scihdr)
sciivar      = mrdfits(file_fits, 1)
waveimg   = mrdfits(file_fits, 2)
objstruct = mrdfits(file_fits, 3)

xatv, diff*sqrt(sciivar)*(waveimg GT 0.0) $
      , wvimg = waveimg, min = -7.0, max = 7.0

IF KEYWORD_SET(objstruct) THEN BEGIN 
   ...
   ENDIF ELSE BEGIN
      ivar  = objstruct[ind].IVAR_BOX
      sig = (ivar GT 0.0)/sqrt(ivar + (ivar LE 0.0))
      wave = objstruct[ind].WAVE_BOX
      flux = objstruct[ind].FLUX_BOX
   ENDELSE
