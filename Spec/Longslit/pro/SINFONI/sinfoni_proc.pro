...
;
; PURPOSE:
;   Process an SINFONI near-IR image
;
; CALLING SEQUENCE:
;   sinfoni_proc, filename, [ flux, invvar, hdr= $
...
pro sinfoni_proc, filename, imag, invvar, hdr = hdr $
...
;; SINFONI Rockwell Hawaii 2RD detector 2048x2048
;; These numbers are from section 9.7 "Detector Characteristics"
;; of the SINFONI manual
GAIN = 2.42 ;; e-/ADU
RDNOISE = 7.0d      ;; this is what manual says for DIT 100s
DARK_CURRENT = 0.15 ;; e-/s, I don't see this noise 
...
imag1 = reverse(xmrdfits(filelist[0], 0, /silent), 2)

exptime = double(sxpar(hdr, 'EXPTIME'))
N_DC =  DARK_CURRENT*exptime
RN_eff = sqrt(N_DC + RDNOISE^2) ;; I'm not sure this is correct
;; BUG: Need to fix the read noise here for other read modes
...
;; I'm treating the Dark current here as just an additional
;; source of photons with Poisson distribution. Check that this is correct
IF arg_present(invvar) THEN invvar = 1.0/(abs(imag - sqrt(2.0)*RN_eff) +RN_eff^2)
;ENDIF
