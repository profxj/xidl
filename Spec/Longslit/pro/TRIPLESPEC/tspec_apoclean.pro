...
function tspec_apoclean, exposure, header, ivar=ivar, sigma=sigma, _extra=extra
...
; Convert to double for algorhythms
  ...
  if keyword_set(CROSS_TALK) then begin
...
; Start with right quadrant
  ...
  dnsamp = double((strsplit(sxpar(header, 'NFOWLER'), /extract))[0])
  rnoise = 18.0D/sqrt(dnsamp)
