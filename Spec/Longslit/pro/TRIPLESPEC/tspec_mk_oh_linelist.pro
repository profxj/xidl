...
;-
;------------------------------------------------------------------------------
pro tspec_mk_oh_linelist, modelfile, linefile
  ...
  stop
  x_fndpeaks, model, peak, NSIG = 5., /silent, PKWDTH = pkwdth, /THIN  $
              , TOLER = TOLER, AUTOFIT=autofit, RMS=rms
  ;print, peak[0:10]
  ;x_splot, model, xtwo=peak, ytwo=model[round(peak)], /bloc, psym2=1
  ;stop

  pkwave = interpol(wave, dindgen(n_elements(wave)), peak)
  x_splot, wave, model, xtwo=pkwave, ytwo=model[round(peak)], /bloc, psym2=1

  label= replicate(' OH', n_elements(peak))
  writecol, linefile, pkwave, model[round(peak)], label
