...
; CALLING SEQUENCE:
;   
;   modelfit  = x_fitgap(image, invvar, maskimage, $
...
;   maskimage- integer mask returned by x_ordermask
;
; RETURNS:
...
;   ncol = (size(image))[1]
;   nrow = (size(image))[2]
;   maskimage      = x_ordermask(ncol, nrow, ordr_str, trim=1.5)
;   scatteredlight = x_fitgap(tflat, tflativar, maskimage)
;
...
function x_fitgap, tflat, tflativar, maskimage, $
           nxbkpt=nxbkpt, nybkpt=nybkpt, maxdev=maxdev

  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'fit = x_fitgap(tflat, tflativar, maskimage, NXBKPT=, ' + $
        'NYBKPT=, MAXDEV= [v1.1]'
      return,-1
  endif
...
;      
  ...
  ;; Cosmic ray
;  svtfi = tflativar
;  gd = where(maskimage LT 0 and tflativar GT 0)
;  djs_iterstat, tflat[gd], median=medi, sigma=sigi
;  cr = where(tflat[gd] GT (medi+5*sigi), ncr)
;  if ncr NE 0 then tflativar[gd[cr]] = 0.
  ...
  for i=0,sz[1]-1 do begin
      ...
      if n GT 2.0*nxbkpt then begin
          ...
          temp = bspline_iterfit(float(inthegap), tflat[inthegap,i], $
                                 ...
                                 fullbkpt=fullbkpt, $
                                 nord=nord, /groupbadpix, maxrej=10, $
                                 upper=3, lower=3);, grow=1, yfit=yfit)
;          tmpfit[*,i] = bspline_valu(findgen(sz[0]),temp)
;          if i mod 100 EQ 0 then begin
;              x_splot, float(inthegap), tflat[inthegap,i], ytwo=yfit, /bloc
;          endif
;             stop
...
;  xatv, tmpfit, /bloc
;  bad= where(maskimage GE 0 or tflativar LE 0.)
;  tmp2 = tflat
;  tmp2[bad] = 0.
;  stop
...
;
  ...
  if keyword_set(svtfi) then tflativar= svtfi
