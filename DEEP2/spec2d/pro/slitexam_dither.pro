pro slitexam_dither,  file, result, doffset, lineparams=lineparams, noplot=noplot, $
...
;----------------------------------------------------------------------
  ...
  shifts=doffset(*,0)
  shifts=round(shifts)
  if min(shifts) LT 0 then shifts-=min(shifts)  
  growfac=max(shifts)-min(shifts)+1

  flux_scale=doffset(*,1)
  seeing = doffset(*,2)
...
;               else badsky=(badsky OR (1b-skymask))
           if n_elements(badsky) eq 0 then begin
               
            dims=size(skymask,/dimension)
            badsky = make_array(dims[0]+4096L*growfac,/byte,value=0b)
            shiftmin=shifts(i-1)*4096L;
            shiftmax=shifts(i-1)*4096L+dims[0]-1
            badsky[shiftmin:shiftmax] = (1b - skymask) 
          endif else begin
            dims=size(skymask,/dimension)
            tmpbadmask = make_array(dims[0]+4096L*growfac,/byte,value=0b)
            shiftmin=shifts(i-1)*4096L;
            shiftmax=shifts(i-1)*4096L+dims[0]-1
            tmpbadmask[shiftmin:shiftmax] =  (1b-skymask)
            badsky = tmpbadmask OR badsky
         endelse
...
;       if useinfo then begin
	 if (i eq 1) then begin
            dims=size(data_in.infomask,/dimension)
            infomask = make_array(dims[0],dims[1]+growfac,/byte,value=0b)
            shiftmin=shifts(i-1);
            shiftmax=shifts(i-1)+dims[1]-1
            infomask[*,shiftmin:shiftmax] = byte(data_in.infomask) 
          endif else begin
            dims=size(data_in.infomask,/dimension)
            tmpinfomask = make_array(dims[0],dims[1]+growfac,/byte,value=0b)
            shiftmin=shifts(i-1);
            shiftmax=shifts(i-1)+dims[1]-1
            tmpinfomask[*,shiftmin:shiftmax] = byte(data_in.infomask) 
            infomask = tmpinfomask OR infomask
         endelse
...
;try masking edge of slit to account for imperfect extraction
  ...
  for sksub=0, nexp-1 do begin
    ...
    data[sksub].ivar[*,1]=0;
    data[sksub].ivar[*,2]=0;
    data[sksub].ivar[*,ydatasize-3]=0;
    data[sksub].ivar[*,ydatasize-2]=0;
    data[sksub].ivar[*,ydatasize-1]=0;
  endfor
...
;create new array to hold shifted data and varience maps
 ...
 for sksub=0, nexp-1 do begin
   ...
   pixmask[*,1,sksub]=0;
   pixmask[*,2,sksub]=0;
   pixmask[*,ydatasize-3,sksub]=0;
   pixmask[*,ydatasize-2,sksub]=0;
   pixmask[*,ydatasize-1,sksub]=0;
 endfor


 ;subtract the sky and shift data for dithers
  for sksub=0, nexp-1 do begin
      shiftmin=shifts(sksub);
      shiftmax=shifts(sksub)+ydatasize-1
;      print,shiftmin,shiftmax
      clean_shifted[*,shiftmin:shiftmax,sksub]=(clean[*,*,sksub]-mediansky.flux)*flux_scale[sksub]
      ivar_shifted[*,shiftmin:shiftmax,sksub]=ivar[*,*,sksub]/(flux_scale[sksub]*flux_scale[sksub])
...
if n_elements(baddata) gt 0 then begin
   ...
   IF signal_rms GT 0.0 THEN BEGIN
     signal_rms=sigma(signal_proj(where(signal_proj lt signal_rms*2 and signal_proj gt -2*signal_rms))) ;sigma cliping
     signal_rms=sigma(signal_proj(where(signal_proj lt signal_rms*2 and signal_proj gt -2*signal_rms))) ;sigma cliping
     maskrows = where(signal_proj gt signal_rms*2)
   ENDIF ELSE BEGIN
     maskrows = -99999
   ENDELSE
...
;create new array to hold object masked data
  ...
  sky_mask=make_array(dims,/float,value=0)
;mask the data and put it into the new array
  for sksub=0, nexp-1 do begin
    ...
    ;atv,clean[*,*,sksub]
    ;atv,clean_masked[*,*,sksub]
  endfor


  
;make the new sky with the masks
  mediansky_final=avsigclip(clean_masked,ivar_masked,1,1,inmask=sky_mask)
 
 
;writefits,'sky.fits',mediansky.flux
;writefits,'masked.fits',clean_masked
...
;create new array to hold shifted data and varience maps
  ...
  dims[1]=dims[1]+growfac
  clean_shifted=make_array(dims,/float,value=0)
  clean_shifted_nosky=make_array(dims,/float,value=0)
  ivar_shifted=make_array(dims,/float,value=0)
  pixmask_shifted=make_array(dims,/float,value=0)
  ivar=data.ivar
  pixmask=1b-baddata
 ...
 ;subtract the sky and shift data for dithers
  for sksub=0, nexp-1 do begin
      shiftmin=shifts(sksub)
      shiftmax=shifts(sksub)+ydatasize-1
      clean_shifted[*,shiftmin:shiftmax,sksub]=(clean[*,*,sksub]-mediansky_final.flux)*flux_scale[sksub]
      clean_shifted_nosky[*,shiftmin:shiftmax,sksub]=(clean[*,*,sksub])*flux_scale[sksub]
      ivar_shifted[*,shiftmin:shiftmax,sksub]=ivar[*,*,sksub]/(flux_scale[sksub]*flux_scale[sksub])
      pixmask_shifted[*,shiftmin:shiftmax,sksub]=pixmask[*,*,sksub]
    endfor
;writefits,'tst.fits',mediansky.flux
...
;  final = avsigclip(clean2,ivar2,inmask=pixmask2)
...
;   endelse
  ...
  if nexp gt 2 then final.ivar=final.ivar*(nbad lt nexp-1)
...
;  bitmask = bitmask*1 OR (avg.mask NE 0)*128 OR (avg.mask EQ 2^nexp-1)*256
  ...
  lambda = lambda_eval_dither(data[0],growfac,shifts,/double) ;get full solution
  dlambda = floatcompress(lambda-lambda[*, 0]#(fltarr(nrows + growfac)+1.))
  ...
  if polyflag then begin
     result =  { flux:floatcompress(final.flux), $
		 ...
		 infomask: infomask ,fluxscale: flux_scale, $
                 offsets: shifts, rawsize: nrows, dithersize: growfac, expseeing: seeing, seeing: meanseeing}
  endif else begin
     result =  { flux:floatcompress(final.flux), $
                 rawflux:floatcompress(final_nosky.flux), $
                 sky:floatcompress(mediansky_final.flux), $
                 ivar:floatcompress(final.ivar), $
                 spflux: floatcompress(data.flux), $
                 spivar: floatcompress(data.ivar), $
                 spmask: pixmask, $
                 mask:newbitmask, crmask:byte(final.mask AND 255B), $
                 lambda0:float(lambda[*, 0]), dlambda:dlambda, $
