...
;-
;------------------------------------------------------------------------------
function deimos_arcfit_withsky, slitarc, arcsat, arcline_x, lamps_in, wset, $
...
;make pretty plots
  if keyword_set(plot) then begin 
     ...
     ;read, waitstring, prompt='Hit enter to continue'
  endif 
...
;      plot,lamps.lambda,dev,xrange=[5000,11000],yrange=[-10,10]
;       plot,lamps.lambda,pxx
;       print,lamps.lambda
;       print,xx3
;       print,pxx
      bad = where(abs(dev) gt maxerr, nbad) ;flag bad line
;       mxerr = sigma(dev)*2
;       if mxerr gt 1   then mxerr = 1
;       if mxerr lt 0.2 then mxerr = 0.2

;     print,"Sigma clip= ", mxerr
;     bad = where(abs(dev) gt mxerr, nbad) ;flag bad line
