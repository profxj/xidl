...
;
; PURPOSE:
;   Generate pixel-to-pixel response flat
;
; CALLING SEQUENCE:
;   deimos_pixflat, files,path
; 
; INPUTS:
...
; done setting up vignetting mask...
	...
	; Make sure N divides 4096!
     ...
     for j=0, n_elements(files)-1 do begin
        ...
        data[*, *, j] = djs_maskinterp(tempimage, $
               ((deimos_badchip AND 2b) EQ 2b), iaxis=0)
	delvarx,tempimage
...
; just use one fix for the whole column.  The below code does the job, though.
	...
	for section=0,nsection - 1 do begin
		...
		for k=0,2047 do begin
                   colmask = deimos_badchip[k, *] AND 3b
...
; mask out regions around 'dust' for good measure
	...
	delvarx,discreppix,badlyvig,vigarr,meddata
     ...
     invdata = mdata/ndata   ; == mdata^2/data
     delvarx, mdata
...
;     flat = 1./flat
     ...
     whbad=where((deimos_badchip AND 3b) NE 0 ,badct)
	...
	flat[0:nfix-1,4095-nfix:4095]=1.
	flat[2048-nfix:2047,4096-nfix:4095]=1.

	
	mwrfits, floatcompress(flat*mask-1., ndig=10), maskfilename
