pro slitexam_dither_tst,  file, result, doffset, growfac, lineparams=lineparams, noplot=noplot, $
...
if n_elements(baddata) gt 0 then begin
   ...
   signal_rms=sigma(signal_proj(where(signal_proj lt signal_rms*1.5 and signal_proj gt -1.5*signal_rms))) ;sigma cliping
   signal_rms=sigma(signal_proj(where(signal_proj lt signal_rms*1.5 and signal_proj gt -1.5*signal_rms))) ;sigma cliping
...
;  bitmask = bitmask*1 OR (avg.mask NE 0)*128 OR (avg.mask EQ 2^nexp-1)*256
  ...
  dlambda = floatcompress(lambda-lambda[*, 0]#(fltarr(nrows + growfac*2 + 1)+1.))
  ...
  if polyflag then begin
     result =  { flux:floatcompress(final.flux), $
		 ...
		 infomask: infomask ,fluxscale: flux_scale, $
                 offsets: shifts, rawsize: nrows, rawoffset: growfac}
  endif else begin
     result =  { flux:floatcompress(final.flux), $
                 rawflux:floatcompress(final_nosky.flux), $
                 sky:floatcompress(mediansky_final.flux), $
