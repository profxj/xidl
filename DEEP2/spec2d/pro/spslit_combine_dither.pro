...
pro spslit_combine_dither, filelist, shifts, result, nlsky = nlsky
;shifts=[26,-25,0,16,-20,-1,1]
...
; this is the workhorse of combing frames 
       ...
       ;minshift=min(shifts)
       ;growfac=
       ;if(-1*minshift gt maxshift) then begin 
       ;    growfac=-1.0*minshift 
       ;endif else begin
       ;    growfac=maxshift
       ;endelse

;modified to call dither reduction rather than regular
       slitexam_dither, filelist[ifile], result, shifts, lineparams=lineparams, /noplot, $
         totalexp=totalexp
       fileout = prefix + 'slit.' 
...
;     endif 
        ...
        if nlsky then begin
          slitexam_dither, filelist[ifile], resultnl, /NLSKY, /noplot
          if n_elements(resultnl.slitfn) gt 1 then $
            mwrfits, resultnl, fileout, header, /no_comment
        endif

;        dhead=headfits(fileout,ext=1) 
;        ddata=mrdfits(fileout,1)
;        dfilename=fileout + '.data'
;        print,dfilename
;        mwrfits,ddata,dfilename,dhead, /no_comment
