; $Id: x_makehtml.pro,v 1.1 2004-05-27 17:27:20 xavier Exp $
;
...
;----------------------------------------------------------------------------
PRO  xmakehh_grab_hdr,name,dict,infile_indx,libfile_indx,txt_file,verbose,$
     strict, CATEG=categ
;
...
;----------------------------------------------------------------------------
PRO xmakehh_gen_file,dict,txt_file,infiles,libfiles,outfile,verbose,title,strict,LinkCode
;
...
;
 ...
 ;; Grab uniq categories
 ucat = dict[ uniq( dict.category, sort(dict.category) )].category
 ncat = n_elements(ucat)

 ;; Loop on categories
 for ii=0L,ncat-1 do begin
     mtch = where(dict[indices].category EQ ucat[ii], nmtch)
     ;; Index.
     PRINTF,final_file,'<A NAME="ROUTINELIST'+string(ii,'(i2)')+'">'
     PRINTF,final_file,'<H1>List of '+ucat[ii]+'</H1></A>'
     PRINTF,final_file,'<UL>'
     for jj=0L,nmtch-1 do begin
         entry = dict[indices[mtch[jj]]]
...
;----------------------------------------------------------------------------
PRO x_makehtml, sources, outfile, VERBOSE=verbose, TITLE=title, $
                STRICT=strict, LINK_FILES=LinkFiles
...
;    a = findfile(src+tok+'*')
;    len = strlen(a)
;;    gd = where( strmid(a,3,/reverse_offset) EQ '.pro', npro)
;    stop
    IF (npro GT 0) THEN BEGIN
     infiles = [infiles,flist]
     categ = [categ, replicate(src, npro)]
 ...
 FOR i=0,count-1 DO BEGIN
     ...
     IF (istlb[i]) THEN BEGIN
         ...
         IF (lib_total EQ 0) THEN libfiles = files $
         ELSE libfiles = [libfiles, files]
         FOR j=0, lib_count-1 DO BEGIN
             name = FILEPATH('mkhtmlhelp.scr',/TMP)
             SPAWN,'LIBRARY/TEXT/EXTRACT='+files[j]+'/OUT='+name+' '+src
             makehh_grab_hdr,files[j],dict,i,lib_total+j,txt_file, $
               verbose,strict
         ENDFOR
     ENDIF ELSE BEGIN
         name = infiles[i]
         xmakehh_grab_hdr,name,dict,i,-1L,txt_file,verbose,strict, CATEG=categ[i]
     ENDELSE
 ENDFOR

;; Generate the HTML file.
xmakehh_gen_file,dict,txt_file,infiles,libfiles,outfile,verbose,title,strict,LinkFiles
END
