...
;   estimate S2N for DEIMOS observations
;
; CALLING SEQUENCE:
;
; INPUTS:
...
;;;
pro deimos_s2ngui_initcommon
;
common deimos_s2ngui_cmmn, $
  s2n_fstrct
end
...
pro deimos_s2ngui_event, ev
  ...
  case uval of
      ...
      'GRATING': begin
          widget_control, state.grating_id, get_value=atmp
          ;; Update Resolution
          tmp = atmp[ev.index]
          x_initdeimos, tmp_instr, GRATING=tmp
          state.str_instr.R = tmp_instr.R
          resolu = state.str_instr.R / ( state.str_instr.swidth / $
                                         state.str_instr.scale_para)
          state.str_instr.grating = tmp
          ;; GUI
          widget_control, state.resd_id, set_value=resolu
          deimos_s2ngui_updplot, state
       end
      'CWAVE': begin
          widget_control, state.cwave_id, get_value=atmp
          ;; Update Resolution
          tmp = atmp[ev.index]
          state.str_instr.cwave = float(tmp)
          deimos_s2ngui_updplot, state
       end
      'PLOT': begin
          state.flg_plot = ev.index
          deimos_s2ngui_gets2n, state
          deimos_s2ngui_updplot, state
      end
      'DWV': begin
          widget_control, state.dwv_id, get_value=tmp
          state.dwv = tmp
          deimos_s2ngui_setwv, state
...
pro deimos_s2ngui_gets2n, state
...
pro deimos_s2ngui_updplot, state
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; 
pro deimos_s2ngui_reinit, state
  ;; READ, DARK
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 
pro deimos_s2ngui, infil=infil, XSIZE=xsize,  YSIZE=ysize
...
;  Optional Keywords
  ...
  deimos_s2ngui_initcommon

  ;; Initialize
  x_initdeimos, str_instr, INFIL=infil, STR_TEL=str_tel, SLIT=slit
...
;    WIDGET
  base = WIDGET_BASE( title = 'deimos_s2ngui: Check spectra', /column, $
...
;;;;;; Info window ;;;;;;;;;;;
  ...
  dumi = widget_text(state.top_id, value='DEIMOS S/N GUI v1.0')
  ...
  ;; Grating
  gratinginf = widget_base(instrinf, /column, /align_center, frame=2)
  state.grating_id = widget_droplist(gratinginf, value=state.gratings, $
                                   title='Grating', uvalue='GRATING')
  RIDX = 1L
  widget_control, state.grating_id, set_droplist_select=RIDX
  state.str_instr.grating = state.gratings[RIDX]

  state.cwave_id = widget_droplist(gratinginf, value=state.cwave, $
                                   title='Central Wave:', uvalue='CWAVE')
  CIDX = 2L
  widget_control, state.cwave_id, set_droplist_select=CIDX
  state.str_instr.cwave = float(state.cwave[CIDX])
  ...
  ;; May wish to generalize this for all WMKO instruments
