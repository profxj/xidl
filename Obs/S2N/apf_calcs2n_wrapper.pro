...
;                         spatialbinning=spatialbinning,spectralbinning=spectralbinning,mag=mag,airmass=airmass,seeing=seeing,exptime=exptime, $
  ...
  if (keyword_set(slitwidth)) then begin
     if slitwidth EQ "N" then decker = "N"
     if slitwidth EQ "S" then decker = "S"
     if slitwidth EQ "M" then decker = "M"
  endif 
 if (keyword_set(slitwidth)) then begin
    x_initapfspec, str_instr, flg,  INFIL=infil, STR_TEL=str_tel, DECKER=slitwidth
 endif else begin
    x_initapfspec, str_instr, flg,  INFIL=infil, STR_TEL=str_tel, DECKER="W"
 endelse
