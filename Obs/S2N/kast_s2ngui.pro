...
;;;
pro kast_s2ngui_initcommon
;
common kast_s2ngui_cmmn, $
...
pro kast_s2ngui_event, ev
  ...
  case uval of
      ...
      'GRISM': begin
          ...
          ;; Update Resolution
          tmp = atmp[ev.index]
          x_initkast, tmp_instr, GRISM=tmp
      ...
      'PLOT': begin
          ...
          kast_s2ngui_gets2n, state
          kast_s2ngui_updplot, state
      end
      'DWV': begin
          widget_control, state.dwv_id, get_value=tmp
          state.dwv = tmp
          kast_s2ngui_setwv, state
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 
pro kast_s2ngui, infil=infil, XSIZE=xsize,  YSIZE=ysize
...
;  Optional Keywords
  ...
  kast_s2ngui_initcommon

  ;; Initialize
  x_initkast, str_instr, INFIL=infil, STR_TEL=str_tel, SLIT=slit
  str_obs = x_obsinit(infil)
  str_obs.seeing = 1.5  ; Ouch!

; STATE

;  readcol, getenv('XIDL_DIR')+'Obs/S2N/THRU_PUT_DATA/kast_setups.dat', $
...
;    WIDGET
  base = WIDGET_BASE( title = 'kast_s2ngui: Check spectra', /column, $
...
;;;;;; Info window ;;;;;;;;;;;
  ...
  dumi = widget_text(state.top1_id, value='Kast S/N GUI v1.1')
...
;  state.config_id = widget_droplist(dispb, value=state.configs, $
  ...
  x_initkast, tmp_instr, grism=state.bdispersers[bidx], grating=state.rdispersers[ridx]
