...
; CALLING SEQUENCE:
;   
;  apf_extract, apf, setup, obj_id, [exp], /RESCHK, /CHK
;
; INPUTS:
;   apf    -  MIKE structure
;   setup   -  Setup ID
...
;             SNR data)
;   /READSKY --  Use the saved sky model from apf_skysub
...
pro apf_extract, apf, setup, obj_id, exp, STD=std, $
                   MSKTRIM=msktrim, $
...
;
  if  N_params() LT 3 then begin 
      print,'Syntax - ' + $
        'apf_extract, apf, setup, obj_id, [exp], ' + $
...
;  Find all relevant obj
  ...
  if nindx EQ 0 then begin
      print, 'apf_extract: No images to find obj for!', obj_id
      return
  endif
...
;  Loop
  ...
  for q=0L,n_elements(exp)-1 do begin
      ...
      ;; Offset ordr_str
      ordr_str = sv_ostr
      ordr_str.lhedg = ordr_str.lhedg +  apf[thisone].arc_xyoff[0]
      ordr_str.rhedg = ordr_str.rhedg +  apf[thisone].arc_xyoff[0]
      ordr_str.lhc[0] = ordr_str.lhc[0] +  apf[thisone].arc_xyoff[0]
      ordr_str.rhc[0] = ordr_str.rhc[0] +  apf[thisone].arc_xyoff[0]

;      rnd_edg = [[round(ordr_str.lhedg)], [round(ordr_str.rhedg)]]
;      sz = size(rnd_edg, /dimensions)
;      rnd_edg = reform(rnd_edg,sz[0],nordr,2)

      print, 'apf_extract: Reading files...'

      ;;;;;;;;;;;;;;
      ;; Open Obj file
      objfil = apf[indx[exp[q]]].obj_fil
      if x_chkfil(objfil+'*') EQ 0 then begin
          print, 'apf_extract: No Obj file! ', objfil, ' Skipping...'
          stop
          continue
      endif
      ...
      ;;;;;;;;;;;;;;
      ;; SKY SUB Fil 
      imgfil =apf_getfil('fin_fil', /name, CHKFIL=chkf, $
                           FRAME=apf[thisone].frame)
      if x_chkfil(imgfil+'*') EQ 0 then begin
          print, 'apf_extract: No Image file!  Returning...'
          return
      endif
      print, 'apf_extract: Image file -- ', imgfil
...
;      var = 1./(ivar + (ivar EQ 0)) * (ivar GT 0)
      ...
      if (size(skysub))[0] EQ 0 then begin
        ...
        print, 'Either run apf_skysub or apf_extract with the ' + $
          '/skipskysub option'
        continue
      endif
      ...
      hel_corr = 1.0d
      helio = 0.
      arc_img = strtrim(apf[indx[exp[q]]].arc_img,2)
      print, 'apf_extract: Arc -- ', arc_img
      if x_chkfil(arc_img+'*') EQ 0 then begin
          print, 'apf_extract: No Arc file!  Returning...', arc_img
          return
      endif
      img_arc = xmrdfits(arc_img, /silent) 
      
      ;; Shift arc?
      rnshft = round(apf[thisone].arc_xyoff[0])
      if rnshft NE 0 then begin
          print, 'apf_extract:  Shifting the Arc by ', rnshft, ' pixels'
          ;; Shift
      ...
      if not keyword_set( NOVAC ) then begin
          print, 'apf_extract: Converting to vacuum wavelengths'
      ...
      ;;  HELIO correction
      x_radec, apf[indx[exp[q]]].ra, apf[indx[exp[q]]].dec, radeg, decdeg 
      ;; JXP Removed the -1 (3/21/05)  ;; Put back!! (5/30/05)
      if not keyword_set(NOHELIO) then begin
          helio = (-1.)*x_keckhelio(radeg, decdeg, $
                                    apf[indx[exp[q]]].equinox, $
                                    jd=apf[indx[exp[q]]].date)
          print, 'apf_extract: heliocentric correction :', $
      ...
      if keyword_set( readsky ) then begin
          skyfil =apf_getfil('sky_fil', /name, CHKFIL=chkf, $
                               FRAME=apf[thisone].frame)
      endif         
      ...
      ;;
      velpix = 1.00 * apf[indx[exp[q]]].rowbin

      ;; Slit length
      ;slit_len = apf_slitlen(apf[thisone].decker)
      case apf[thisone].decker of
         '2.0x7.5': slit_len = 7.5
         else: stop
      endcase

      ;; QAFIL
      qafil = apf_getfil('qa_extract', setup, FRAME=apf[thisone].frame)

      ;; Flat (for blaze normalization)
      if keyword_set(EXFLAT) then begin
          flat_img = apf_getfil('qtz_fil', setup, FIL_NM=flat_fil)
...
;      ordr_str = ordr_str[0:2]
      ...
      if not keyword_set(GAUSS) and not keyword_set(NOPROF) then begin
          ...
          x_extechopt, img_sub, skysub, sciivar, ordr_str, objstr, velpix, $
                           ...
                           RDNOISE=apf[indx[exp[q]]].readno, $
                           OBJ_NAME=apf[indx[exp[q]]].Obj, $
...
;          if keyword_set(DEGRADE) then ivar = ivar * DEGRADE
          ...
          ;; Ouptut Spectra
          print, 'apf_extract: Outputting boxcar spectrum-- ', objfil
          mwrfits, objstr, objfil, /create, /silent
          spawn, 'gzip -f '+objfil
          
          ;; Output the image
          print, 'apf_extract: Outputting skysub image-- ', objfil
;          apf_taghead, head
...
;      restore, 'debug16B.idl'
      ...
      ordermask = x_ordermask(sz_img[0], sz_img[1], ordr_str, trim=OMSKTRIM, /noextra)
      ;; Loop on orders
...
;      IORDR = 11L
      for qq=IORDR,nordr-1 do begin
          print, 'apf_extract: Extracting order ', $
          ...
          if keyword_set(MODEL_PROF) then begin
              ...
              if max(tprof) LT 0.001 then begin
                  continue
                  print, 'apf_extract: No profile.  Skipping..'
              endif
          endif
          ...
          ;; Npix
          npix = round(4608. / apf[indx[exp[q]]].rowbin) 
          ...
          if not keyword_set(BOXCAR) then begin
              ...
              x_extobjopt, timg, tarc, tvar, tsky, $
                ...
                READNO=apf[indx[exp[q]]].readno, DEFGAU=defgau, $
          ...
          ;; Write to structure
          spec = total(timg,2)
          ;x_splot, spec, ytwo=fin_spec.fx, /bloc
...
;                          objstr[sci].aper = fin_spec.aper
      ...
      ;; Ouptut Spectra
      print, 'apf_extract: Output spectrum in -- ', objfil
      mwrfits, objstr, objfil, /create, /silent
      spawn, 'gzip -f '+objfil
