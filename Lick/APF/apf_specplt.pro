...
; CALLING SEQUENCE:
;   
;   apf_specplt, ydat, [head], XSIZE=, YSIZE=, TITLE=, WAVE=, LLIST=,
;           QAL=, ERR=, /GAL
;
; INPUTS:
;   ydat       - Values 
...
common apf_specplt_cmmn, apfobj
          ...
          case ev.type of
              0 : begin ; Button press
                  case ev.press of
                      ...
                      4 : apf_specplt_SetLine, state   ; Set reference line
                      else: 
                  endcase
              end
...
;                set_value='Expecting another s !!'
          ...
          case eventch of
              ...
              ; SMOOTH
              'S': apf_specplt_smooth, state
              'U': apf_specplt_smooth, state, /reset
              ; Crude Analysis
              'E': apf_specplt_EW, state        ; Calc EW
              'N': x_specplot_Colm, state      ; Calc AODM colm
              ' ': print, 'x: '+strtrim(state.xpos,2)+$
                '   y:'+strtrim(state.ypos,2)
              ; LINES
              'L': apf_specplt_SetLine, state   ; Set reference line
              'A': begin ; Plot AlIII
              ...
              ;; ORDERS
              '0': apf_specplt_Chgordr, state, 0
              '1': apf_specplt_Chgordr, state, 1
              '2': apf_specplt_Chgordr, state, 2
              '3': apf_specplt_Chgordr, state, 3
              '4': apf_specplt_Chgordr, state, 4
              '5': apf_specplt_Chgordr, state, 5
              '6': apf_specplt_Chgordr, state, 6
              '7': apf_specplt_Chgordr, state, 7
              '8': apf_specplt_Chgordr, state, 8
              '9': apf_specplt_Chgordr, state, 9
              '=': apf_specplt_Chgordr, state, /add
              '-': apf_specplt_Chgordr, state, /sub
              ; Postscript
              'P': apf_specplt_psfile, state  
              ; QUIT
              'q': begin
                  widget_control, ev.top, /destroy
                  return
              end
              else:  print, 'apf_specplt: Not a valid key!' ; Nothing
          endcase
      end
      'DONE' : begin
          widget_control, ev.top, /destroy
...
; Update Plot
  if not keyword_set(NOUPD) then apf_specplt_UpdatePlot, state
...
pro apf_specplt_UpdatePlot, state
...
; Plot Data
  ...
  if flg_lines EQ 1 then apf_specplt_PltLines, state
  
  ; EW
  if state.flg_EW NE 0 then apf_specplt_PltEW, state
...
pro apf_specplt_Reset, state

common apf_specplt_cmmn

  case state.flg_fspec of 
      0: begin ; Box
          ;; Set wave, fx
;          a = where(apfobj[state.ordr].box_fx NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
;          amin = (a[0] > 0)
;          if amin GT 100 then amin = 0L
;          bmax = b[nb-1]
;          if bmax LT apfobj[state.ordr].npix/2 then $
;            bmax = 4999 else bmax = a[na-1]
;          npix = bmax-amin+1
;          state.npix = npix
          amin = 0L
          bmax = apfobj[state.ordr].nrow-1
          state.npix = bmax+1
          npix = state.npix
          
          state.wave[0:npix-1] = apfobj[state.ordr].box_wv[amin:bmax]
          state.fx[0:npix-1] = apfobj[state.ordr].box_fx[amin:bmax] 
          state.sig[0:npix-1] = sqrt(apfobj[state.ordr].box_var[amin:bmax])
      end
      1: begin ; Optimal
          ;; Set wave, fx
;          a = where(apfobj[state.ordr].wave NE 0., na, $
;                    COMPLEMENT=b, NCOMPLEMENT=nb)
...
;          endelse
          amin = 0L
          bmax = apfobj[state.ordr].npix - 1
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = apfobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = apfobj[state.ordr].fx[amin:bmax]
          state.sig[0:npix-1] = sqrt(apfobj[state.ordr].var[amin:bmax] > 0.)
      end
      2: begin ; FSPEC
          ;; Set wave, fx
          ido = where(state.ordr EQ apfobj.phys_ordr, na)
          if na EQ 0 then stop
          a = where(apfobj.wave[*,ido] NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          if na EQ 0 then begin
              amin = 0L
              bmax = 100L
...
;              if amin GT 100 then amin = 0L
          ...
          state.wave[0:npix-1] = apfobj.wave[amin:bmax,ido]
          state.fx[0:npix-1] = apfobj.fx[amin:bmax,ido]
          state.sig[0:npix-1] = sqrt(apfobj.var[amin:bmax,ido] > 0.)
      end
      3: begin ; fluxed
          ;; Set wave, fx
          a = where(apfobj[state.ordr].wave NE 0., na, $
          ...
          state.wave[0:npix-1] = apfobj[state.ordr].wave[amin:bmax]
          state.fx[0:npix-1] = apfobj[state.ordr].flux[amin:bmax]
          state.sig[0:npix-1] = apfobj[state.ordr].sig[amin:bmax]
      end
      4: begin ; Sky
          ;; Set wave, fx
          a = where(apfobj[state.ordr].sky NE 0., na, $
                    COMPLEMENT=b, NCOMPLEMENT=nb)
          amin = (a[0] > 0)
          if amin GT 100 then amin = 0L
          bmax = b[nb-1]
          if bmax LT apfobj[state.ordr].nrow/2 then $
            bmax = 4999 else bmax = a[na-1]
          npix = bmax-amin+1
          state.npix = npix
          
          state.wave[0:npix-1] = apfobj[state.ordr].sky_wv[amin:bmax]
          state.fx[0:npix-1] = apfobj[state.ordr].sky[amin:bmax] 
...
pro apf_specplt_PltLines, state, IMG=img
...
pro apf_specplt_Colm, state

common x_specplot_lines

  ; Check for error array
  if state.flg_sig NE 1 then begin
      print, 'apf_specplt_Colm: Need to specify the error array!'
      return
  endif
...
pro apf_specplt_guess, state, val, IMG=img
...
pro apf_specplt_smooth, state, RESET=reset
...
pro apf_specplt_Chgordr, state, newordr, ADD=add, SUB=sub
...
common apf_specplt_cmmn
  ...
  if keyword_set( ADD ) or keyword_set( SUB ) then begin
      if keyword_set( ADD ) then begin
          ...
          if state.flg_fspec NE 2 then begin
              if newordr EQ n_elements(apfobj) then newordr = 0
          endif else begin
              if apfobj.phys_ordr[newordr] LE 0 then begin
                  pos = where(apfobj.phys_ordr GT 0, npos)
                  newordr = pos[0]
              endif
          endelse
          state.ordr = newordr
      ...
      if keyword_set( SUB ) then begin
          newordr = state.ordr - 1
          if state.flg_fspec NE 2 then begin
              if newordr LT 0 then newordr = n_elements(apfobj)-1
          endif else begin
              if apfobj.phys_ordr[newordr] LE 0 then begin
                  pos = where(apfobj.phys_ordr GT 0, npos)
                  newordr = pos[npos-1]
  ...
  case state.flg_fspec of
      0: state.phys_ordr = apfobj[state.ordr].order 
      1: state.phys_ordr = apfobj[state.ordr].order 
      2: state.phys_ordr = apfobj.phys_ordr[state.ordr]
      3: state.phys_ordr = apfobj[state.ordr].order 
      4: state.phys_ordr = apfobj[state.ordr].order 
      else: stop
  endcase

  ;; RESET
  widget_control, state.ordr_id, set_value=state.phys_ordr
  apf_specplt_Reset, state
...
pro apf_specplt, spec_fil, obj_id, XSIZE=xsize, YSIZE=ysize, $
...
;
;  print,'Syntax - ' + $
;    'apf_specplt, spec_fil, obj_id, XSIZE=,YSIZE=, /NOCTB, WAVE=, '
...
; Input the Obj structure
  if not keyword_set( FSPEC ) then begin
      ...
      apf_wrfspec, objstr, spec_fil, /read, UVES=uves 
      gdobj = [0L]
      ;; Initial order
...
; PHYSORDR
  if not keyword_set( FSPEC ) then state.phys_ordr = apfobj[state.ordr].order $
    else state.phys_ordr = apfobj.phys_ordr[state.ordr]
...
;    WIDGET
  base = WIDGET_BASE( title = 'apf_specplt', /column, $
...
; Send to the xmanager
  if not keyword_set(BLOCK) then xmanager, 'apf_specplt', base, /no_block $
  else xmanager, 'apf_specplt', base
